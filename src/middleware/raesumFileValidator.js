...
const raesumFileValidationMiddleware = async (req, res, next) => {
    ...
    logger.debug(
        `Starting synchronous file validation middleware`,
        Date.now() - start
    );
    ...
    logger.debug(
        `File type specified: ${req.body.fileType}`,
        Date.now() - start
    );
    ...
    let fileTypeDetails;
    try {
        fileTypeDetails = await raesumFile.getOneFileType(req.body.fileType);
        if (!fileTypeDetails) {
            logger.info(
                `Invalid file type: ${req.body.fileType}`,
                Date.now() - start
            );
            const message = await raesumResponses.get('fileTypeNotAllowed');
            return res.status(message.code).json(message);
        ...
        logger.debug(
            `File type details retrieved for: ${req.body.fileType}`,
            Date.now() - start
        );
        ...
        if (!req.file || !req.file.mimetype) {
            logger.debug(
                `No file or mimetype found in request`,
                Date.now() - start
            );
            const message = await raesumResponses.get('fileTypeNotAllowed');
            return res.status(message.code).json(message);
        ...
        const isMimeTypeValid = await raesumFileValidator.validateMime(
            req.file.mimetype,
            req.body.fileType
        );
        if (!isMimeTypeValid) {
            logger.info(
                `Mimetype ${req.file.mimetype} does not match expected type for ${req.body.fileType}`,
                Date.now() - start
            );
            const message = await raesumResponses.get('fileTypeNotAllowed');
            return res.status(message.code).json(message);
        ...
        logger.debug(
            `Mimetype validation passed for ${req.file.mimetype}`,
            Date.now() - start
        );
    } catch (e) {
        logger.error(
            `Error getting file type details: ${e.message}`,
            Date.now() - start
        );
        const message = await raesumResponses.get('fileTypeNotAllowed');
        return res.status(message.code).json(message);
    ...
    try {
        ...
        const isSizeValid = await raesumFileValidator.validateSize(
            req.file.size,
            req.body.fileType
        );
        if (!isSizeValid) {
            logger.debug(
                `File size ${req.file.size} exceeds limit for ${req.body.fileType}`,
                Date.now() - start
            );
            const message = await raesumResponses.get('fileSizeTooLarge');
            return res.status(message.code).json(message);
        ...
        logger.debug(
            `File size validation passed for size: ${req.file.size}`,
            Date.now() - start
        );
    } catch (e) {
        logger.error(
            `Error validating file size: ${e.message}`,
            Date.now() - start
        );
        const message = await raesumResponses.get('fileSizeTooLarge');
        return res.status(message.code).json(message);
    ...
    try {
        ...
        if (
            ...
            if (!req.file.width || !req.file.height) {
                logger.debug(
                    `No dimensions found in request for file type requiring dimensions`,
                    Date.now() - start
                );
            ...
            const isImageMimeType = raesumFileValidator.validateImageMime(
                req.file.mimetype
            );
            if (!isImageMimeType) {
                logger.info(
                    `File type ${req.file.mimetype} is not a known image type`,
                    Date.now() - start
                );
                const message = await raesumResponses.get('fileTypeNotAllowed');
                return res.status(message.code).json(message);
            ...
            const areDimensionsValid =
                await raesumFileValidator.validateDimensions(
                    req.file.width,
                    req.file.height,
                    req.body.fileType
                );
            if (!areDimensionsValid) {
                logger.debug(
                    `Dimensions ${req.file.width}x${req.file.height} exceed limit for ${req.body.fileType}`,
                    Date.now() - start
                );
                const message = await raesumResponses.get(
                    'fileDimensionsTooLarge'
            ...
            logger.debug(
                `Dimensions validation passed for ${req.file.width}x${req.file.height}`,
                Date.now() - start
            );
        } else {
            logger.debug(
                `No dimension validation required for ${req.body.fileType}`,
                Date.now() - start
            );
        }
    } catch (e) {
        logger.error(
            `Error validating file dimensions: ${e.message}`,
            Date.now() - start
        );
