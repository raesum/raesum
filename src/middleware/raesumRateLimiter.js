...
const raesumLimiterMiddleware = async (req, res, next) => {
    ...
    if (
        raesumRateLimiter.pathWeights &&
        raesumRateLimiter.pathWeights[req.path]
    ) {
        let pointsToConsumeCandidate = parseInt(
            raesumRateLimiter.pathWeights[req.path]
        );
        ...
        logger.verbose(
            `Using path-specific weight ${pointsToConsume} for path ${req.path}`,
            Date.now() - start
        );
    }

    logger.verbose(
        `Rate limit consuming ${pointsToConsume} points) for ${key} on path ${req.path}`,
        Date.now() - start
    );
