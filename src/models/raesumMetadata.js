...
class raseumMetadataObject {
    ...
    async set(key, value) {
        ...
        if (
            ...
            throw new Error(
                'Metadata value must be a string, number, or boolean'
            );
        }

        const query =
            'INSERT INTO raesum_metadata (datakey, datavalue) VALUES ($1, $2) ON CONFLICT (datakey) DO UPDATE SET datavalue = $2';
    ...
    async getByKey(key) {
        ...
        const query =
            'SELECT datavalue FROM raesum_metadata WHERE datakey = $1';
        const result = await raesumDB.query(query, [key]);

        if (result.rows.length === 0) {
            logger.warning(
                'Metadata not found by key: ' + key,
                Date.now() - start
            );
        ...
        if (parseInt(result.rows[0].datavalue) == result.rows[0].datavalue) {
            ...
            parseFloat(result.rows[0].datavalue) == result.rows[0].datavalue
        ) {
    ...
    async delete(key) {
        ...
        if (result.rowCount === 0) {
            logger.warning(
                'Metadata not found for deletion: ' + key,
                Date.now() - start
            );
            return false;
        }
