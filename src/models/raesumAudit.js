...
class raesumAuditObject {
    ...
    async #init() {
        ...
        this.#objectTypesByStringKey =
            raesumAuthorization.objectTypesByStringKey;
    ...
    async create(actionType, objectType, objectID = null, userID) {
        ...
        if (objectID && (isNaN(objectID) || objectID < 1) && objectID != null) {
            logger.error(
                'Non-numeric input for objectID logEvent',
                Date.now() - start
            );
            throw new Error('Invalid format for objectID or userID');
        }
        ...
        if (typeof actionType == 'string') {
            actionType =
                await raesumAuthorization.convertActionStringToID(actionType);
        }
        ...
        const query =
            'INSERT INTO raesum_audit_log (action_id, object_type_id, object_id, user_id) VALUES ($1, $2, $3, $4);';
        const curvalQuery =
            "SELECT currval(pg_get_serial_sequence('raesum_audit_log','id'));";
        ...
        try {
            ...
            logger.verbose(
                `Audit log entry added: ${insertedResult['rows'][0].currval}`,
                Date.now() - start
            );
            return parseInt(insertedResult['rows'][0].currval);
        } catch (e) {
            logger.error(
                'Error adding audit log entry with error: ' + e,
                Date.now() - start
            );
    ...
    async getAuditLogs(
        ...
        try {
            ...
            logger.error(
                `Organization ID ${orgID} does not exist: ${error.message}`,
                Date.now() - start
            );
        ...
        if (userID) {
            try {
                ...
                if (user.current_organization_id !== orgID) {
                    logger.error(
                        `User ID ${userID} does not belong to organization ${orgID}`,
                        Date.now() - start
                    );
                    throw new Error(
                ...
                logger.error(
                    `User ID ${userID} validation failed: ${error.message}`,
                    Date.now() - start
                );
                throw new Error(
                    'User does not exist or does not belong to this organization'
                );
            }
        }
        ...
        if (objectTypeID && (isNaN(objectTypeID) || objectTypeID < 1)) {
            logger.error(
                'Invalid objectTypeID for getAuditLogs',
                Date.now() - start
            );
        ...
        if (objectTypeID) {
            try {
                ...
                const validObjectTypeIds = Object.values(
                    this.#objectTypesByStringKey
                );
                if (!validObjectTypeIds.includes(objectTypeID)) {
                    logger.error(
                        `Object type ID ${objectTypeID} does not exist`,
                        Date.now() - start
                    );
                ...
                logger.error(
                    `Object type ID ${objectTypeID} validation failed: ${error.message}`,
                    Date.now() - start
                );
        ...
        if (actionTypeID && (isNaN(actionTypeID) || actionTypeID < 1)) {
            logger.error(
                'Invalid actionTypeID for getAuditLogs',
                Date.now() - start
            );
        ...
        if (actionTypeID) {
            try {
                ...
                const validActionTypeIds = Object.values(
                    this.#actionsByStringKey
                );
                if (!validActionTypeIds.includes(actionTypeID)) {
                    logger.error(
                        `Action type ID ${actionTypeID} does not exist`,
                        Date.now() - start
                    );
                ...
                logger.error(
                    `Action type ID ${actionTypeID} validation failed: ${error.message}`,
                    Date.now() - start
                );
        ...
        try {
            ...
            logger.info(
                `Retrieved ${result.rows.length} audit logs for org ${orgID}`,
                Date.now() - start
            );
            return result.rows;
        } catch (error) {
