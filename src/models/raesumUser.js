...
class raesumUserObject {
    ...
    async createUser(
        ...
        logger.verbose(
            `Attempting to create user with username: ${username} for org: ${currentOrganizationId}`,
            Date.now() - start
        );
        ...
        logger.debug(
            `Checking if username: ${username} is unique`,
            Date.now() - start
        );
        let user = null;
        ...
        if (user) {
            ...
            if (
                ...
                logger.warning(
                    'Attemted to create user with cognitoID ' +
                        external_id +
                        ' but user already exists with matching username and cognitoID'
                );
                return user.id;
            }
        ...
        logger.debug(
            `Checking if external_id: ${external_id} is unique`,
            Date.now() - start
        );
        ...
        if (user) {
            logger.error(
                `Cannot create new user. ExternalID: ${external_id} is not unique`,
                Date.now() - start
            );
        ...
        const addedToOrg = await raesumOrganization.addUserToOrganization(
            newUserID,
            currentOrganizationId
        );
    ...
    async getUserByUsername(username) {
        ...
        try {
            ...
            logger.verbose(
                `Warning getting user by username: ${username} with error: ` +
                    e,
                Date.now() - start
            );
            throw new Error('User not found');
        ...
        if (theId && !isNaN(theId) && theId > 0) {
            logger.debug(
                `User with username: ${username} found with ID: ${theId}`,
                Date.now() - start
            );
    ...
    async getUserByExternalID(external_id) {
        ...
        logger.verbose(
            `Getting user by external_id: ${external_id}`,
            Date.now() - start
        );
        ...
        try {
            ...
            if (result.rows.length > 0) {
                ...
                logger.verbose(
                    `User with external_id: ${external_id} not found`,
                    Date.now() - start
                );
                throw new Error('User not found');
            }
            ...
            logger.verbose(
                'Error getting user by external_id: ' + e,
                Date.now() - start
            );
        ...
        if (theId && !isNaN(theId) && theId > 0) {
            ...
            logger.warning(
                `User with username: ${external_id} not found`,
                Date.now() - start
            );
            throw new Error('User not found');
        }
    ...
    async setActivationStatus(id, activeStatus) {
        ...
        logger.debug(
            `Setting activation status for user with ID: ${id} to: ${activeStatus}`,
            Date.now() - start
        );
        ...
        try {
            ...
            logger.debug(
                `Database activation status set for user with ID: ${id} to: ${activeStatus}`,
                Date.now() - start
            );
        } catch (e) {
        ...
        try {
            logger.debug(
                `Updating Cognito status for user: ${user.username} to: ${activeStatus}`,
                Date.now() - start
            );
            await raesumCognito.setUserEnabledStatus(
                user.username,
                activeStatus
            );
            logger.info(
                `Activation status set for user with ID: ${id} to: ${activeStatus} (including Cognito)`,
                Date.now() - start
            );
            return true;
        } catch (cognitoError) {
            logger.error(
                `Failed to update Cognito status for user ${user.username}: ${cognitoError.message}`,
                Date.now() - start
            );
            ...
            try {
                ...
                logger.warning(
                    `Rolled back database activation status for user with ID: ${id} due to Cognito failure`,
                    Date.now() - start
                );
            } catch (rollbackError) {
                logger.error(
                    `Failed to rollback database change for user ${id}: ${rollbackError.message}`,
                    Date.now() - start
                );
            }

            throw new Error(
                `Failed to update user status in Cognito: ${cognitoError.message}`
            );
        }
    }
    ...
    async initRaesum(organizationID) {
        ...
        const firstUserEmail = await raesumConfig.get(
            'initialization.firstUserEmail'
        );
        const firstUserRole = await raesumConfig.get(
            'initialization.firstUserRole'
        );
        const firstUserPassword = await raesumConfig.get(
            'initialization.firstUserPassword'
        );
        ...
        if (!organizationID || isNaN(organizationID) || organizationID < 1) {
            organizationID = 1;
            logger.debug(
                `Initializing: No organizationID supplied, setting to: ${organizationID}`,
                Date.now() - start
            );
        }
        logger.debug(
            `Initializing: Creating Default User for Organization ID: ${organizationID}`,
            Date.now() - start
        );
        ...
        try {
            logger.verbose(
                `Creating user in Cognito: ${firstUserUsername}`,
                Date.now() - start
            );
            cognitoUserId = await raesumCognito.createCognitoUser(
                firstUserUsername,
                firstUserEmail,
                firstUserPassword,
                true
            ...
            logger.info(
                `User created in Cognito with ID: ${cognitoUserId}`,
                Date.now() - start
            );
        } catch (cognitoError) {
            logger.error(
                `Failed to create user in Cognito: ${cognitoError.message}`,
                Date.now() - start
            );
            throw new Error(
                `Failed to create initial user in Cognito: ${cognitoError.message}`
            );
        }
        ...
        logger.info(
            `User with ID: ${userID} created in local database`,
            Date.now() - start
        );
        ...
        try {
            logger.info(
                `Assigning user with ID: ${userID} to role: ${firstUserRole} or orgID 1`,
                Date.now() - start
            );
    ...
    async getAllowedUserOrgs(userID) {
        ...
        logger.verbose(
            `Getting allowed organizations for user with ID: ${userID}`,
            Date.now() - start
        );
        ...
        try {
            ...
            logger.error(
                'Error getting allowed organizations: ' + e,
                Date.now() - start
            );
            throw new Error('Error getting allowed organizations');
        }
        logger.verbose(
            `${orgs.length} allowed organizations for user with ID: ${userID} found`,
            Date.now() - start
        );
        return orgs;
    }
    ...
    async changeUserOrg(userID, orgID) {
        ...
        logger.verbose(
            `Changing user with ID: ${userID} to organization with ID: ${orgID}`,
            Date.now() - start
        );
        ...
        if (!allowedOrgs.includes(orgID)) {
            logger.warning(
                `User with ID: ${userID} is not allowed to switch to organization with ID: ${orgID}. OrgID or UserID might not exist.`,
                Date.now() - start
            );
        ...
        try {
            ...
            logger.info(
                `User with ID: ${userID} changed to organization with ID: ${orgID}`,
                Date.now() - start
            );
            return true;
        } catch {
    ...
    async clearMetadataKeyCache() {
        ...
        logger.verbose(
            'User Metadata key list cache cleared',
            Date.now() - start
        );
    }
    ...
    async getMetadataKeys(show_inactive = false) {
        ...
        if (cachedKeys && Object.keys(cachedKeys).length > 0) {
            logger.verbose(
                `Returning cached metadata keys`,
                Date.now() - start
            );
            logger.debug(
                `Key list metadata with ${Object.keys(cachedKeys).length} keys: ${JSON.stringify(cachedKeys)}`,
                Date.now() - start
            );
            return cachedKeys;
        }
        ...
        logger.debug(
            'Got ' + response.rows.length + ' metadata keys from database',
            Date.now() - start
        );
        ...
        if (!cacheResponse) {
            logger.error(
                'Failed to save metadata keys to cache',
                Date.now() - start
            );
        } else {
            logger.debug(
                'Saved ' + response.rows.length + ' metadata keys to cache',
                Date.now() - start
            );
        }

        let cognitoKeys = {};
        const allCognitoKeys = [];
        const readOnlyCognitoKeys = [];

        // Loop through the keys and build the cognito key status object
        for (const key of keyList) {
            // If the key is a cognito key, add it to the cognitoKey list
            if (keys[key].cognito_attribute) {
                allCognitoKeys.push(keyList[key]);

                // If the key is writable, add it to the writable cognito keys list
                if (keys[key].cognito_writable) {
                    readOnlyCognitoKeys.push(keyList[key]);
                }
            }
        }

        cognitoKeys = {
            cognitoKeys: allCognitoKeys,
            readOnlyCognitoKeys: readOnlyCognitoKeys,
        };
        ...
        // Return object
        logger.verbose(
            `Returning ${response.rows.length} metadata keys`,
            Date.now() - start
        );
        return keys;
    }
    ...
    async getMetadataKeyList(show_inactive = false) {
        ...
        logger.debug(
            'Checking cache for user metadata key list',
            Date.now() - start
        );
        ...
        if (!cachedKeys || Object.keys(cachedKeys).length === 0) {
            logger.verbose(
                `User Metadata key list not found in cache, building it`,
                Date.now() - start
            );
        ...
        logger.verbose(
            `Returning cached user metadata key list`,
            Date.now() - start
        );
        logger.debug(
            `Key list user metadata with ${cachedKeys.length} keys: ${JSON.stringify(cachedKeys)}`,
            Date.now() - start
        );
        return cachedKeys;
    }
    ...
    async getMetadataCognitoKeyStatus() {
        ...
        if (!cognitoKeys) {
            ...
            for (const key of keyList) {
                ...
                if (keys[key].cognito_attribute) {
                    ...
                    // If the key is writable, add it to the writable cognito keys list
                    if (keys[key].cognito_writable == false) {
                        readOnlyCognitoKeys.push(keyList[key]);
                    }
                }
            }
        ...
        logger.verbose(
            `Returning list of keys controlled by cognito`,
            Date.now() - start
        );
     ...
     */
    async setUserMetadataKey(
        key,
        description = '',
        activeStatus = true,
        cognitoAttribute = false,
        cognitoWritable = false
    ) {
        const start = Date.now();
        ...
        if (!key.match(/^[a-zA-Z0-9\-\_\.]+$/)) {
            logger.warning(
                `Unable to create user metadata key: ${key} contains invalid characters`,
                Date.now() - start
            );
        ...
        const keys = await this.getMetadataKeyList(true);
        logger.info(
            `Creating/setting user metadata key: ${key}`,
            Date.now() - start
        );
        ...
        if (keys.includes(key)) {
            ...
            try {
                ...
                logger.error(
                    `Error creating user metadata key: ${key}`,
                    Date.now() - start
                );
    ...
    async deleteUserMetadataKey(key) {
        ...
        if (result.rows[0].count > 0) {
            logger.warning(
                `User metadata key: ${key} is in use and cannot be deleted`,
                Date.now() - start
            );
            return false;
        }
        ...
        // Delete the key
        const sql =
            'DELETE FROM raesum_user_metadata_keys WHERE datakey ILIKE $1';
        try {
            logger.info(
                `Deleting user metadata key: ${key}`,
                Date.now() - start
            );
            ...
            logger.error(
                `Error deleting user metadata key: ${key}`,
                Date.now() - start
            );
    ...
    async getUserMetadataValues(userId, keys, activeStatus = false) {
        ...
        logger.debug(
            `Geting User Metadata Key Values for user ${userId} and keys ${keys.join(',')} before invalid keys removed`,
            Date.now() - start
        );
        logger.debug(
            `Valid user metadata keys ${validKeys.join(',')}`,
            Date.now() - start
        );
        ...
        keys = keys.filter((key) =>
            validKeys
                .map((validKey) => validKey.toLowerCase())
                .includes(key.toLowerCase())
        );
        logger.verbose(
            `Geting User Metadata Key Values for user ${userId} and keys ${keys.join(',')}`,
            Date.now() - start
        );
        ...
        try {
            ...
            logger.info(
                `Retrieved ${response.rows.length} user metadata values for keys ${keys.join(', ')}`,
                Date.now() - start
            );
            return values;
        } catch (e) {
            throw new Error(
                'Error getting user metadata values',
                Date.now() - start
            );
        }
    }
    ...
    async setUserMetadataValues(
        ...
        // Get the list of user metadata values
        const validKeys = await this.getMetadataKeyList(activeStatus);
        const cognitoKeysAll = await this.getMetadataCognitoKeyStatus();
        const cognitoKeys = cognitoKeysAll.cognitoKeys;
        const readOnlyCognitoKeys = cognitoKeysAll.readOnlyCognitoKeys;
        ...
        const existingValues = await this.getUserMetadataValues(
            userId,
            keyList,
            activeStatus
        );
        ...
        if (newKeyList.length > 0) {
            ...
            sql +=
                'INSERT INTO raesum_user_x_metadata (user_id, datakey, value) VALUES ';
        ...
        try {
            ...
            logger.error(
                `Failed to insert new matadata values for user: ${e.message}`,
                Date.now() - start
            );
        ...
        for (let q = 0; q < updateKeyList.length; q++) {
            ...
            try {
                await raesumDB.query(sql, [
                    values[updateKeyList[q]],
                    userId,
                    updateKeyList[q],
                ]);
            } catch (e) {
                logger.error(
                    `Failed to update user metadata values: ${e.message}`,
                    Date.now() - start
                );
        ...
        if (updateCognito) {
            ...
            for (const key of keyList) {
                ...
                if (
                    cognitoKeys.includes(key) &&
                    !readOnlyCognitoKeys.includes(key)
                ) {
            ...
            if (cognitoAttributesToUpdate.length > 0) {
                try {
                    ...
                    await raesumCognito.updateCognitoUserAttributes(
                        user.username,
                        cognitoAttributesToUpdate
                    );
                    logger.info(
                        `Successfully updated ${cognitoAttributesToUpdate.length} Cognito attributes for user ${user.username}`,
                        Date.now() - start
                    );
                } catch (cognitoError) {
                    ...
                    for (const attr of cognitoAttributesToUpdate) {
                        logger.warning(
                            `Failed to update Cognito attribute '${attr.Name}' for user ${user.username}: ${cognitoError.message}`,
                            Date.now() - start
                        );
                    }
     ...
     * @throw {Error} If the user does not exist
     */
    async deleteUserMetadataValues(
        userId,
        keys,
        activeStatus = false,
        updateCognito = true
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (!doNotDelete.includes(key)) {
                ...
                logger.warning(
                    `Attempting to delete mandatory metadata ${key} from user ${userId}`,
                    Date.now() - start
                );
            }
        }
        ...
        keysToDelete = keysToDelete.filter(
            (key) =>
                validKeys.includes(key) && !readOnlyCognitoKeys.includes(key)
        );

        if (keysToDelete.length === 0) {
            logger.verbose(
                `No valid keys to delete for user ${userId}`,
                Date.now() - start
            );
            return true;
        }
        ...
        try {
            ...
            logger.info(
                `Deleted ${keysToDelete.length} metadata values for user ${userId}`,
                Date.now() - start
            );
        } catch (e) {
            logger.error(
                `Error deleting user metadata values for user ${userId}: ${e.message}`,
                Date.now() - start
            );
        ...
        if (updateCognito) {
            ...
            if (cognitoAttributesToDelete.length > 0) {
                try {
                    await raesumCognito.deleteCognitoUserAttributes(
                        user.username,
                        cognitoAttributesToDelete
                    );
                    logger.info(
                        `Successfully deleted ${cognitoAttributesToDelete.length} Cognito attributes for user ${user.username}`,
                        Date.now() - start
                    );
                } catch (cognitoError) {
                    ...
                    for (const attr of cognitoAttributesToDelete) {
                        logger.warning(
                            `Failed to delete Cognito attribute '${attr}' for user ${user.username}: ${cognitoError.message}`,
                            Date.now() - start
                        );
                    }
                    // Don't throw - we still want to return true since the Raesum DB was updated
    ...
    async syncUserFromCognitoToRaesum(username) {
        ...
        logger.verbose(
            `Getting user from Cognito: ${username}`,
            Date.now() - start
        );
        let cognitoUser;
        try {
            ...
            logger.error(
                `User ${username} not found in AWS Cognito: ${error.message}`,
                Date.now() - start
            );
        ...
        if (external_id == '') {
            logger.error(
                'Cannot continue to sync user metadata to Raesum,external ID is empty',
                Date.now() - start
            );
        ...
        try {
            ...
            logger.verbose(
                `User ${external_id} found in Raesum with ID: ${raesumUserId}`,
                Date.now() - start
            );
        } catch (error) {
            // If no user is found, create the user
            logger.info(
                `User ${external_id} not found in Raesum, creating new user`,
                Date.now() - start
            );
            try {
                const newUserDefaults =
                    await raesumConfig.get('newUserDefaults');
                ...
                logger.info(
                    `Created new user in Raesum for Cognito user ${external_id} with ID: ${raesumUserId}`,
                    Date.now() - start
                );
            } catch (createError) {
                logger.error(
                    `Failed to create user in Raesum: ${createError.message}`,
                    Date.now() - start
                );
                throw new Error(
                    `Failed to create user in Raesum: ${createError.message}`
        ...
        if (Object.keys(valuesToSet).length > 0) {
            try {
                await this.setUserMetadataValues(
                    raesumUserId,
                    valuesToSet,
                    true,
                    false
                ...
                logger.info(
                    `Synced ${Object.keys(valuesToSet).length} metadata values for user ${external_id}`,
                    Date.now() - start
                );
            } catch (metadataError) {
                logger.error(
                    `Failed to set metadata values for user ${external_id}: ${metadataError.message}`,
                    Date.now() - start
                );
