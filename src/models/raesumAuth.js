...
class raesumAuthorizationObject {
    ...
    async init() {
        ...
        if (
            Object.keys(this.actionsByStringKey).length == 0 ||
            Object.keys(this.objectTypesByStringKey).length == 0
        ) {
            ...
            actions['rows'].forEach((action) => {
                this.actionsByStringKey[action.string_key.toLowerCase()] =
                    action.id;
                this.actionIDs.push(action.id);
            });
            logger.info(
                Object.keys(this.actionsByStringKey).length + ' actions loaded',
                Date.now() - start
            );
            ...
            objectTypes['rows'].forEach((objectType) => {
                this.objectTypesByStringKey[
                    objectType.string_key.toLowerCase()
                ] = objectType.id;
                this.objectIDs.push(objectType.id);
            });
            logger.info(
                Object.keys(this.objectTypesByStringKey).length +
                    ' object types loaded',
                Date.now() - start
            );
            ...
            scopes['rows'].forEach((scope) => {
                this.scopesByStringKey[scope.string_key.toLowerCase()] =
                    scope.id;
                this.scopesIDs.push(scope.id);
            });
            logger.info(
                Object.keys(this.scopesByStringKey).length + ' scopes loaded',
                Date.now() - start
            );
        }
    }
    ...
    async loadGlobalRolesToDatabase() {
        ...
        const dirPath = path.join(
            ...
            '..',
            '..',
            'controlledData/authorization/'
        );
        const jsonFile = fs.readFileSync(
            path.join(dirPath, 'globalRoles.json'),
            'utf8'
        );
        ...
        for (let i = 0; i < jsonData.length; i++) {
            ...
            if (Object.keys(jsonData[i]).length > 0) {
                ...
                const roleQuery =
                    'SELECT * FROM raesum_auth_role WHERE string_key = $1';
                const roleResult = await raesumDB.query(roleQuery, [
                    jsonData[i].string_key,
                ]);
                ...
                if (roleResult.rows.length == 0) {
                    logger.verbose(
                        `Role with key: ${jsonData[i].string_key} does not exist, creating role`,
                        Date.now() - start
                    );
                    const roleInsertQuery =
                        'INSERT INTO raesum_auth_role (string_key, name, active_status) VALUES ($1, $2, $3) RETURNING id';
                    const roleInsertResult = await raesumDB.query(
                        roleInsertQuery,
                        [
                            jsonData[i].string_key,
                            jsonData[i].name,
                    ...
                    // Update the role
                    logger.verbose(
                        `Role with key: ${jsonData[i].string_key} exists, updating role`,
                        Date.now() - start
                    );
                    roleID = roleResult.rows[0].id;
                    const roleUpdateQuery =
                        'UPDATE raesum_auth_role SET name = $1, active_status = $2 WHERE id = $3';
                    await raesumDB.query(roleUpdateQuery, [
                        jsonData[i].name,
                        jsonData[i].active_status,
                        roleID,
                    ]);
                ...
                if (jsonData[i].restrict_to_organization.length > 0) {
                    ...
                    const orgRestrictionsArr = jsonData[
                        i
                    ].restrict_to_organization.filter((el) => {
                        return typeof el == 'number';
                    });

                    // Insert new restrictions if any are INT
                    if (orgRestrictionsArr.length > 0) {
                        const orgRestrictionsString =
                            orgRestrictionsArr.join(',');
                    ...
                    try {
                        ...
                        logger.verbose(
                            `Set org restrictions set for role: ${jsonData[i].name} with key: ${jsonData[i].string_key}`,
                            Date.now() - start
                        );
                    } catch (e) {
                        logger.error(
                            `Error setting org restrictions for role: ${jsonData[i].name} with key: ${jsonData[i].string_key}. Error: ${e}`,
                            Date.now() - start
                        );
                    }
                } else {
                    logger.verbose(
                        `No org restrictions for role: ${jsonData[i].name} with key: ${jsonData[i].string_key}`,
                        Date.now() - start
                    );
                ...
                logger.verbose(
                    `Setting permissions for role: ${jsonData[i].name} with key: ${jsonData[i].string_key}`,
                    Date.now() - start
                );
                if (typeof roleID == 'number' && roleID > 0) {
                    ...
                    for (let j = 0; j < objectKeys.length; j++) {
                        ...
                        const objectID = await this.convertObjectTypeStringToID(
                            objectKeys[j]
                        );
                        if (objectID) {
                            const theObject =
                                jsonData[i].permissions[objectKeys[j]];
                            ...
                            for (let k = 0; k < permissionKeys.length; k++) {
                                ...
                                const permissionID =
                                    await this.convertActionStringToID(
                                        permissionKeys[k]
                                    );
                                ...
                                if (permissionID) {
                                    ...
                                    for (
                                        let s = 0;
                                        s < theObject[permissionKeys[k]].length;
                                        s++
                                    ) {
                                        // Convert the scope string key into a number
                                        const scopeID =
                                            await this.convertScopeStringToID(
                                                theObject[permissionKeys[k]][s]
                                            );
    ...
    async checkUserPermission(
        ...
        try {
            objectTypeID =
                await this.convertObjectTypeStringToID(objectTypeString);
            logger.debug(
                'Permission Converted object type string to ID (' +
                    objectTypeString +
                    ')'
            );
        } catch (e) {
            logger.error(
                'Failed to convert object type string (' +
                    objectTypeString +
                    ') to ID: ' +
                    e
            );
        ...
        try {
            ...
            logger.debug(
                'Permission Converted action type string to ID  (' +
                    actionString +
                    ')'
            );
        } catch (e) {
            logger.error(
                'Failed to convert action string (' +
                    actionString +
                    ') to ID: ' +
                    e
            );
        ...
        try {
            if (scopesRequired && Array.isArray(scopesRequired)) {
                scopesRequired = scopesRequired.map((scope) =>
                    this.convertScopeStringToID(scope)
                );
            }
        } catch (e) {
        ...
        logger.debug(
            'Checking user permission for user ' +
                userID +
                ' on object type ' +
                objectTypeString +
                ' with action ' +
        ...
        return await this.checkUserPermissionByID(
            userID,
            objectTypeID,
            actionString,
            orgID,
    ...
    async checkUserPermissionByID(
        ...
        if (typeof orgID != 'number' || orgID < 1) {
            logger.error(
                'Invalid input for checkUserPermissionByID. Must be a positive int.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for checkUserPermissionByID. Must be a positive int.'
        ...
        if (!this.objectIDs.includes(objectTypeId)) {
            logger.error(
                'Invalid input for checkUserPermissionByID. Must be a valid object type ID.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for checkUserPermissionByID. Must be a valid object type ID.'
        ...
        if (!this.actionIDs.includes(actionId)) {
            logger.error(
                'Invalid input for checkUserPermissionByID. Must be a valid action ID.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for checkUserPermissionByID. Must be a valid action ID.'
        ...
        if (
            scopeIDsRequired &&
            Array.isArray(scopeIDsRequired) &&
            scopeIDsRequired.length > 0
        ) {
            logger.debug(
                'Scopes required have been supplied',
                Date.now() - start
            );
            ...
            for (let i = 0; i < scopeIDsRequired.length; i++) {
                if (!this.scopesIDs.includes(scopeIDsRequired[i])) {
                    ...
                    logger.warning(
                        'Requested invalid scope: ' + scopeIDsRequired[i],
                        Date.now() - start
                    );
                    return false;
                }
        ...
        if (scopeIDsRequired.length == 0) {
            logger.debug(
                'No valid scopes required have been supplied',
                Date.now() - start
            );
            ...
            if (userID == objectOwnerUserID && orgID == objectOwnerUserID) {
                logger.debug(
                    'User is the owner of the object and the org matches the object org',
                    Date.now() - start
                );
                scopeIDsRequired = [1, 2, 3];
            } else {
                logger.debug(
                    'User is not the owner of the object. Getting user org to determine if org-level access is allowed',
                    Date.now() - start
                );
                ...
                if (userOrg == orgID) {
                    logger.debug(
                        "User's org matches the object's org. Using org level or global scope",
                        Date.now() - start
                    );
                    ...
                    logger.debug(
                        "User's org does not match the object's org. Using global scope",
                        Date.now() - start
                    );
                    scopeIDsRequired = [3];
                }
        ...
        logger.debug(
            'Checking for deny permission ' + +JSON.stringify(denyparams),
            Date.now() - start
        );
        ...
        logger.debug(
            'Checking for permission ' + JSON.stringify(params),
            Date.now() - start
        );
    ...
    async getAllowedUserScopesByID(userID, objectTypeId, actionId) {
        ...
        if (!this.objectIDs.includes(objectTypeId)) {
            logger.error(
                'Invalid input for getAllowedUserScopesByID. Must be a valid object type ID.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for getAllowedUserScopesByID. Must be a valid object type ID.'
        ...
        if (denyresult.rows.length > 0) {
            logger.info(
                'User has been denied permission due to a deny action',
                Date.now() - start
            );
            return false;
        }
        ...
        logger.debug(
            'getAllowedUserScopesByID Checking allow permissions',
            Date.now() - start
        );
        ...
        if (
            ...
            !isNaN(parseInt(result.rows[0].scope_id))
        ) {
            const allowedScope = parseInt(result.rows[0].scope_id);
            logger.debug(
                'getAllowedUserScopesByID User has allowed scope: ' +
                    allowedScope,
                Date.now() - start
            );
            return allowedScope;
            ...
            logger.debug(
                'getAllowedUserScopesByID No scopes found for user',
                Date.now() - start
            );
            return false;
        }
    ...
    async convertActionStringToID(actionString) {
        ...
        if (typeof actionString != 'string') {
            logger.error(
                'Invalid input for convertActionStringToID. Must be a string.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for convertActionStringToID. Must be a string.'
        ...
        if (this.actionsByStringKey[actionString]) {
            ...
            logger.warning(
                'Unable to find action type with string: ' + actionString,
                Date.now() - start
            );
            return false;
        }
    ...
    async convertObjectTypeStringToID(objectTypeString) {
        ...
        if (typeof objectTypeString != 'string') {
            logger.error(
                'Invalid input for convertObjectTypeStringToID. Must be a string.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for convertObjectTypeStringToID. Must be a string.'
        ...
        if (this.objectTypesByStringKey[objectTypeString]) {
            ...
            logger.warning(
                'Unable to find object type with string: ' + objectTypeString,
                Date.now() - start
            );
            return false;
        }
    ...
    async convertScopeStringToID(scopeString) {
        ...
        if (typeof scopeString != 'string') {
            logger.error(
                'Invalid input for convertScopeStringToID. Must be a string.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for convertScopeStringToID. Must be a string.'
        ...
        if (this.scopesByStringKey[scopeString]) {
            ...
            logger.warning(
                'Unable to find scope type with string: ' + scopeString,
                Date.now() - start
            );
            return false;
        }
    ...
    async getUserRoles(userID, orgID = null, activeOnly = true) {
        const start = Date.now();
        logger.verbose(
            'Starting getUserRoles for user ' + userID,
            Date.now() - start
        );
        ...
        if (typeof userID != 'number' || userID < 1) {
            logger.error(
                'Invalid userID input for getUserRoles. Must be a positive int.',
                Date.now() - start
            );
            throw new Error(
                'Invalid userID input for getUserRoles. Must be a positive int.'
        ...
        if (typeof orgID != 'number' || orgID < 1) {
            logger.debug(
                'OrgID not submitted for getUserRoles. Getting current orgID for user',
                Date.now() - start
            );
            // Get the user
            const user = await raesumUser.getUserById(userID);
        ...
        try {
            ...
            logger.info(
                `Found ${result.rows.length} roles for user: ${userID}`,
                Date.now() - start
            );
            ...
            logger.error(
                `Error getting user roles for user: ${userID}`,
                Date.now() - start
            );
            throw e;
        }
    ...
    async addUserToRole(userID, roleID, orgID) {
        ...
        if (typeof userID != 'number' || userID < 1) {
            logger.error(
                'Invalid input for addUserToRole. Must be a positive int.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for addUserToRole. Must be a positive int.'
        ...
        if (typeof orgID != 'number' || orgID < 1) {
            logger.debug(
                'OrgID not submitted. Getting current orgID for user',
                Date.now() - start
            );
            // Get the user
            const user = await raesumUser.getUserById(userID);
        ...
        logger.debug(
            `Checking if user is already in role: ${userID}, ${roleID}, ${orgID}`,
            Date.now() - start
        );
        try {
            ...
            if (result.rowCount > 0) {
                logger.warning(
                    `User ${userID} already in role: ${roleID} for org: ${orgID}, or role, org, or user does not exist`,
                    Date.now() - start
                );
                return false;
            }
            ...
            logger.error(
                `Error checking if user ${userID} is in role ${roleID} for org ${orgID} with error: ` +
                    e,
                Date.now() - start
            );
        ...
        logger.debug(
            `User is not already in role, attempting to add: ${userID}, ${roleID}, ${orgID}`,
            Date.now() - start
        );
        ...
        try {
            ...
            const result = await raesumDB.query(query, params);
            logger.debug(
                `Result of adding user to role: ${userID}, ${roleID}, ${orgID}`,
                Date.now() - start
            );

            if (result.rowCount > 0) {
                logger.info(
                    `User: ${userID} added to role: ${roleID} for org: ${orgID}`,
                    Date.now() - start
                );
                return true;
            }
            ...
            logger.error(
                `Error adding user ${userID} to role ${roleID} for org ${orgID} with error: ` +
                    e,
                Date.now() - start
            );
    ...
    async addUserToRoleByKey(userID, roleStringKey, orgID) {
        ...
        if (typeof roleStringKey != 'string' || roleStringKey == '') {
            logger.error(
                "Invalid input for addUserToRoleByKey. 'roleStringKey' must be a string.",
                Date.now() - start
            );
            throw new Error(
                "Invalid input for addUserToRoleByKey. 'roleStringKey' must be a string."
        ...
        if (result.rows.length > 0) {
            ...
            logger.verbose(
                `Role found for key ${roleStringKey}. Adding user to role.`,
                Date.now() - start
            );
    ...
    async removeUserFromRole(userID, roleID, orgID) {
        ...
        if (typeof userID != 'number' || userID < 1) {
            logger.error(
                'Invalid input for removeUserFromRole. Must be a positive int.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for removeUserFromRole. Must be a positive int.'
        ...
        try {
            ...
            if (result.rowCount > 0) {
                logger.info(
                    `User: ${userID} removed from role: ${roleID} for org: ${orgID}`,
                    Date.now() - start
                );
                return true;
            } else {
                logger.warning(
                    `User ${userID} not in role: ${roleID} for org: ${orgID}, or role, org, or user does not exist`,
                    Date.now() - start
                );
                return false;
            }
            ...
            logger.error(
                'Error removing user from role with error: ' + e,
                Date.now() - start
            );
    ...
    async getRoleByID(roleID, activeOnly) {
        ...
        logger.debug(
            'getRoleByID starting for roleID ' + roleID,
            Date.now() - start
        );
        // Validate that roleID is a positive int
        if (typeof roleID != 'number' || roleID < 1) {
            logger.error(
                'Invalid input for getRoleByID. Must be a positive int.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for getRoleByID. Must be a positive int.'
    ...
    async getRolesByIDs(roleIDs, activeOnly) {
        ...
        if (result.rows.length > 0) {
            logger.verbose(
                `getRolesByIDs found ${result.rows.length} roles`,
                Date.now() - start
            );
            return result.rows;
        } else {
            logger.warning(
                `Roles with IDs: ${roleIDs.join(',')} not found`,
                Date.now() - start
            );
            throw new Error('Role not found');
        }
    ...
    async getRoleByKey(roleStringKey, activeOnly) {
        ...
        if (typeof roleStringKey != 'string' || roleStringKey == '') {
            logger.error(
                'Invalid input for getRoleByKey. Must be a non-empty string.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for getRoleByKey. Must be a non-empty string.'
        ...
        if (result.rows.length > 0) {
            const result2 = await this.getRoleByID(
                result.rows[0].id,
                activeOnly
            );
            return result2;
            ...
            logger.warning(
                `Role with key: ${roleStringKey} not found`,
                Date.now() - start
            );
            throw new Error('Role not found');
        }
    ...
    async getRolesByID(roleIDArray, activeOnly) {
        ...
        if (!Array.isArray(roleIDArray) || roleIDArray.length == 0) {
            logger.error(
                'Invalid input for getRolesByID. Must be an array with at least one positive int.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for getRolesByID. Must be an array with at least one positive int.'
        ...
        if (result.rows.length > 0) {
            logger.verbose(
                `Found ${result.rows.length} roles`,
                Date.now() - start
            );
            return result.rows;
        } else {
    ...
    async getRolesForOrg(orgID, activeOnly) {
        ...
        if (typeof orgID != 'number' || orgID < 1) {
            logger.error(
                'Invalid input for getRolesForOrg. Must be a positive int.',
                Date.now() - start
            );
            throw new Error(
                'Invalid input for getRolesForOrg. Must be a positive int.'
        ...
        logger.verbose(
            `Found ${roleIDs.length} roles for org: ${orgID}`,
            Date.now() - start
        );
