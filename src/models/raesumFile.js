...
class raseumFileObject {
    ...
    async getFileTypes() {
        ...
        if (cachedTypes && Object.keys(cachedTypes).length > 0) {
            ...
            logger.debug(
                `File types with ${Object.keys(cachedTypes).length} types: ${JSON.stringify(cachedTypes)}`,
                Date.now() - start
            );
            return cachedTypes;
        }
        ...
        logger.debug(
            'Getting file types from controlled data',
            Date.now() - start
        );
        try {
            const fileTypesData =
                await import('../../controlledData/file/allowedUploadTypes.json');
            ...
            logger.debug(
                `Got ${Object.keys(fileTypes).length} file types from controlled data`,
                Date.now() - start
            );
            logger.verbose(
                `Returning ${Object.keys(fileTypes).length} file types`,
                Date.now() - start
            );
            ...
            logger.error(
                `Error getting file types: ${e.message}`,
                Date.now() - start
            );
    ...
    async getOneFileType(fileTypeKey) {
        ...
        logger.debug(
            `Getting file type for key: ${fileTypeKey}`,
            Date.now() - start
        );
    ...
    async updateFileStatus(fileId, status, reason = '') {
        ...
        logger.verbose(
            `Attempting to update file status for fileId: ${fileId} to status: ${status}`,
            Date.now() - start
        );
        ...
        try {
            ...
            logger.error(
                `Error getting status ID: ${e.message}`,
                Date.now() - start
            );
            throw new Error('Invalid status');
        }
        ...
        try {
            ...
            logger.info(
                `File status updated for fileId: ${fileId} to status: ${status}`,
                Date.now() - start
            );
            return true;
        } catch (e) {
            logger.error(
                `Error updating file status: ${e.message}`,
                Date.now() - start
            );
    ...
    async upload(fileId, s3Path, bufferStream, mimetype) {
        ...
        logger.debug(
            `Starting file upload for fileId: ${fileId} to S3 path: ${s3Path}`,
            Date.now() - start
        );
        ...
        try {
            ...
            logger.error(
                `Error getting file record: ${e.message}`,
                Date.now() - start
            );
            throw new Error('File not found');
        }
        ...
        try {
            ...
            logger.warning(
                `Error checking S3 path uniqueness: ${e.message}`,
                Date.now() - start
            );
        }
        ...
        try {
            ...
            logger.info(
                `File record updated for fileId: ${fileId} with status: validating`,
                Date.now() - start
            );
        } catch (e) {
            logger.error(
                `Error updating file record: ${fileId} with error ${e.message}`,
                Date.now() - start
            );
        ...
        this.validateFileAsync(fileId).catch((e) => {
            logger.error(
                `Async validation failed for fileId: ${fileId}: ${e.message}`
            );
        });
    ...
    async getEntryById(fileId) {
        ...
        logger.verbose(
            `Getting file entry by ID: ${fileId}`,
            Date.now() - start
        );
        ...
        try {
            ...
            logger.error(
                `Error getting file entry by ID: ${e.message}`,
                Date.now() - start
            );
    ...
    async getEntriesByUserAndOrg(userId, orgId) {
        ...
        logger.verbose(
            `Getting file entries for userId: ${userId}, orgId: ${orgId}`,
            Date.now() - start
        );
        ...
        try {
            ...
            logger.info(
                `Found ${files.length} file entries`,
                Date.now() - start
            );
            return files;
        } catch (e) {
            logger.error(
                `Error getting file entries by user and org: ${e.message}`,
                Date.now() - start
            );
    ...
    */
    ...
    async getMetaDataKeys(show_inactive = false) {
        ...
        if (cachedKeys && Object.keys(cachedKeys).length > 0) {
            logger.verbose(
                `Returning cached file metadata keys`,
                Date.now() - start
            );
            logger.debug(
                `Key list file metadata with ${Object.keys(cachedKeys).length} keys: ${JSON.stringify(cachedKeys)}`,
                Date.now() - start
            );
            return cachedKeys;
        }
        ...
        logger.debug(
            'Got ' + response.rows.length + ' file metadata keys from database',
            Date.now() - start
        );
        ...
        if (!cacheResponse) {
            logger.error(
                'Failed to save file metadata keys to cache',
                Date.now() - start
            );
        } else {
            logger.debug(
        ...
        // Return object
        logger.verbose(
            `Returning ${response.rows.length} file metadata keys`,
            Date.now() - start
        );
        return keys;
    }
    ...
    async getFileMetadataValues(fileId, keys, activeStatus = false) {
        ...
        if (keys.length === 0) {
            logger.verbose(
                `No valid keys to retrieve for fileId ${fileId}`,
                Date.now() - start
            );
            return {};
        }
        ...
        try {
            ...
            logger.verbose(
                `Retrieved ${Object.keys(metadata).length} metadata values for file ${fileId}`,
                Date.now() - start
            );
            return metadata;
        } catch (e) {
            logger.error(
                `Error getting file metadata values for file ${fileId}: ${e.message}`,
                Date.now() - start
            );
    ...
    async deleteFileMetadataValues(
        fileId,
        keys,
        activeStatus = false,
        forceWrite = false
        ...
        if (keysToDelete.length === 0) {
            logger.verbose(
                `No valid keys to delete for fileId ${fileId}`,
                Date.now() - start
            );
            return true;
        }
        ...
        try {
            ...
            logger.info(
                `Deleted ${keysToDelete.length} metadata values for file ${fileId}`,
                Date.now() - start
            );
        } catch (e) {
            logger.error(
                `Error deleting file metadata values for file ${fileId}: ${e.message}`,
                Date.now() - start
            );
    ...
    async setFileMetadataValues(
        ...
        logger.debug(
            `setFileMetadataValues Attempting to update file metadata`,
            Date.now() - start
        );
        ...
        const existingValues = await this.getFileMetadataValues(
            fileId,
            keyList,
            activeStatus
        );
        ...
        logger.debug(
            `setFileMetadataValues existingValues: ${JSON.stringify(existingValues)} validKeys: ${Object.keys(validKeys).join(', ')} keyList: ${keyList.join(', ')} updateKeyList: ${updateKeyList.join(', ')} newKeyList: ${newKeyList.join(', ')}`,
            Date.now() - start
        );
        ...
        if (newKeyList.length > 0) {
            ...
            sql +=
                'INSERT INTO raesum_file_x_metadata (file_id, datakey, value) VALUES ';
        ...
        try {
            ...
            logger.error(
                `Failed setFileMetadataValues to insert new metadata values for file: ${e.message}`,
                Date.now() - start
            );
            throw new Error(
                'Error setFileMetadataValues inserting new file metadata values'
            );
        }
        ...
        for (let q = 0; q < updateKeyList.length; q++) {
            ...
            try {
                ...
                logger.error(
                    `Failed setFileMetadataValues to update file metadata values: ${e.message}`,
                    Date.now() - start
                );
                throw new Error(
                    'Error setFileMetadataValues updating file metadata values'
                );
            }
        }

        logger.verbose(
            'File setFileMetadataValues completed for file: ' +
                fileId +
                ' with keys ' +
