...
class raesumOrganizationObject {
    ...
    async initRaesum() {
        ...
        logger.info(
            'Initializing: Creating Default Organization',
            Date.now() - start
        );
        ...
        try {
            ...
            if (existingOrg) {
                logger.warning(
                    'Default organization already exists, skipping creation',
                    Date.now() - start
                );
                return 1;
            }
        ...
        return this.create(
            ...
            'This is the default organization for the Raesum system.'
        );
    }
    ...
    async loadGlobalMetadataToDatabase() {
        ...
        const start = Date.now();
        logger.info(
            'Loading Organization Metadata Definitions',
            Date.now() - start
        );
        ...
        const jsonFile = fs.readFileSync(
            path.join(dirPath, 'organizationMetadata.json'),
            'utf8'
        );
        ...
        for (let i = 0; i < jsonData.length; i++) {
            ...
            if (Object.keys(jsonData[i]).length > 0) {
                ...
                if (existingKey.rows.length > 0) {
                    logger.info(
                        'Organization Metadata Key Definition Being Updated: ' +
                            jsonData[i].datakey,
                        Date.now() - start
                    );
                    ...
                    logger.info(
                        'Organization Metadata Key Definition Being Added: ' +
                            jsonData[i].datakey,
                        Date.now() - start
                    );
        ...
        logger.info(
            'Completed adding/updating organization metadata keys',
            Date.now() - start
        );
    ...
    async create(name, activeStatus) {
        ...
        try {
            const query =
                'INSERT INTO raesum_organization (name, active_status) VALUES ($1, $2) RETURNING id';
            const result = await raesumDB.query(query, [name, activeStatus]);
            logger.info(
                `Organization created with id: ${result.rows[0].id}`,
                Date.now() - start
            );
    ...
    async setActivationStatus(id, activeStatus) {
        ...
        logger.info(
            `Setting activation status for org: ${id} to ${activeStatus}`,
            Date.now() - start
        );
        ...
        // Update the org
        const query =
            'UPDATE raesum_organization SET active_status = $1 WHERE id = $2';
        try {
            const orgResult = await raesumDB.query(query, [activeStatus, id]);

            logger.verbose(
                `Activation status set for org: ${id} to ${activeStatus}`,
                Date.now() - start
            );
        } catch (e) {
            logger.error(
                `Error setting activation status for org: ${id} to ${activeStatus} with error: ${e}`,
                Date.now() - start
            );
        ...
        if (activeStatus === false) {
            logger.info(
                `Deactivating org: ${id}. Starting Moving Users.`,
                Date.now() - start
            );
            ...
            logger.info(
                `Found ${orgUsers.length} users on org: ${id} to deactivate or move`,
                Date.now() - start
            );

            for (let i = 0; i < orgUsers.length; i++) {
                logger.verbose(
                    'Organization setActivationStatus processing user: ' +
                        orgUsers[i].id
                );

                // For each user, check if they are part of another org
                const userOrgs = await raesumUser.getAllowedUserOrgs(
                    orgUsers[i].id
                );
                logger.debug(
                    'Organization setActivationStatus userOrgs: ' +
                ...
                if (userOrgs.length > 0) {
                    ...
                    logger.info(
                        `Moving user: ${orgUsers[i].id} to org: ${userOrgs[0]}`,
                        Date.now() - start
                    );
                    ...
                    if (orgUsers[i].active_status === true) {
                        logger.info(
                            `Deactivating user: ${orgUsers[i].id}`,
                            Date.now() - start
                        );
                        try {
                            await raesumUser.setActivationStatus(
                                orgUsers[i].id,
                                false
                            );
                        } catch (e) {
                            logger.error(
                                `Failed to deactivate user: ${orgUsers[i].id}: ${e.message}`,
                                Date.now() - start
                            );
                        }
                    }
            ...
            logger.info(
                `Deactivating org: ${id}. Finished Moving Users.`,
                Date.now() - start
            );
        }

        logger.info(
            `Finished Setting activation status for org: ${id} to ${activeStatus}`,
            Date.now() - start
        );
    ...
    async addUserToOrganization(userID, orgID) {
        ...
        logger.info(
            `Adding user: ${userID} to org: ${orgID}`,
            Date.now() - start
        );
        ...
        try {
            ...
            if (userResult.length === 0) {
                logger.warning(
                    `Cannot add user: ${userID} to org: ${orgID}. User not found`,
                    Date.now() - start
                );
        ...
        try {
            ...
            if (orgResult.active_status === false) {
                logger.warning(
                    `Cannot add user: ${userID} to org: ${orgID}. Org not active or doesn't exist`,
                    Date.now() - start
                );
        ...
        if (checkResult.rows.length > 0) {
            logger.verbose(
                `Cannot add user: ${userID} to org: ${orgID}. User is already part of the org`,
                Date.now() - start
            );
            return true;
        }
        ...
        const query =
            'INSERT INTO raesum_organization_x_user (user_id, org_id) VALUES ($1, $2) ON CONFLICT (user_id, org_id) DO NOTHING;';

        try {
            await raesumDB.query(query, [userID, orgID]);
            logger.verbose(
                `User: ${userID} added to org: ${orgID}`,
                Date.now() - start
            );
            return true;
        } catch (e) {
            logger.error(
                `Error adding user: ${userID} to org: ${orgID} with error: ${e}`,
                Date.now() - start
            );
    ...
    async removeUserFromOrganization(userID, orgID) {
        ...
        logger.info(
            `Removing user: ${userID} from org: ${orgID}`,
            Date.now() - start
        );
        ...
        try {
            ...
            if (userResult.length === 0) {
                logger.warning(
                    `Cannot remove user: ${userID} from org: ${orgID}. User not found`,
                    Date.now() - start
                );
                throw new Error('User not found');
            }
        ...
        try {
            ...
            if (orgResult.active_status === false) {
                logger.warning(
                    `Cannot remove user: ${userID} from org: ${orgID}. Org not active or doesn't exist`,
                    Date.now() - start
                );
                throw new Error('Org is not active or does not exist');
            }
        ...
        const query =
            'DELETE FROM raesum_organization_x_user WHERE user_id = $1 AND org_id = $2';
        try {
            await raesumDB.query(query, [userID, orgID]);
            logger.verbose(
                `User: ${userID} removed from org: ${orgID}`,
                Date.now() - start
            );
        } catch (e) {
            logger.error(
                `Error removing user: ${userID} from org: ${orgID} with error: ${e}`,
                Date.now() - start
            );
    ...
    async getUsers(orgID, activeStatus) {
        ...
        logger.info(
            `Organization getUsers found ${result.rows.length} users for org: ${orgID}`,
            Date.now() - start
        );
        return result.rows;
    }
    ...
    async getMetadataKeys(show_inactive = false) {
        ...
        if (cachedKeys && Object.keys(cachedKeys).length > 0) {
            logger.verbose(
                `Returning cached organization metadata keys`,
                Date.now() - start
            );
            logger.debug(
                `Key list organization metadata with ${Object.keys(cachedKeys).length} keys: ${JSON.stringify(cachedKeys)}`,
                Date.now() - start
            );
            return cachedKeys;
        }
        ...
        logger.debug(
            'Getting organization metadata keys from database',
            Date.now() - start
        );
        ...
        logger.debug(
            'Got ' +
                ...
                ' organization metadata keys from database',
            Date.now() - start
        );
        ...
        if (!cacheResponse) {
            logger.error(
                'Failed to save organization metadata keys to cache',
                Date.now() - start
            );
        } else {
            logger.debug(
                'Saved ' +
                    response.rows.length +
                    ' organization metadata keys to cache',
                Date.now() - start
            );
        }
        ...
        // Return object
        logger.verbose(
            `Returning ${response.rows.length} organization metadata keys`,
            Date.now() - start
        );
        return keys;
    }
    ...
    async getMetadataKeyList(show_inactive = false) {
        ...
        logger.debug(
            'Checking cache for Organization metadata key list',
            Date.now() - start
        );
        ...
        if (!cachedKeys || Object.keys(cachedKeys).length === 0) {
            logger.verbose(
                `getMetadataKeyList Organization Metadata key list not found in cache, building it`,
                Date.now() - start
            );
            ...
            logger.debug(
                `getMetadataKeyList Organization Metadata keys: ${JSON.stringify(keys)}`,
                Date.now() - start
            );
            ...
            logger.debug(
                `getMetadataKeyList Organization Metadata key list found in cache`,
                Date.now() - start
            );
        ...
        logger.verbose(
            `Returning cached Organization metadata key list`,
            Date.now() - start
        );
        logger.debug(
            `Organization Key list metadata with ${cachedKeys.length} keys: ${JSON.stringify(cachedKeys)}`,
            Date.now() - start
        );
        return cachedKeys;
    }
    ...
    */
    async clearMetadataKeyCache() {
        const start = Date.now();
        logger.debug(
            'Clearing Organization metadata key cache',
            Date.now() - start
        );
        ...
        logger.verbose(
            'Organization Metadata key list cache cleared',
            Date.now() - start
        );
    }
    ...
    async getOrganizationMetadataValues(orgId, keys, activeStatus = false) {
        ...
        logger.debug(
            `Geting User Metadata Key Values for user ${orgId} and keys ${keys.join(',')} before invalid keys removed`,
            Date.now() - start
        );
        ...
        logger.verbose(
            `Geting Organization Metadata Key Values for user ${orgId} and keys ${keys.join(',')}`,
            Date.now() - start
        );
        ...
        try {
            ...
            logger.info(
                `Retrieved ${response.rows.length} organization metadata values for keys ${keys.join(', ')}`,
                Date.now() - start
            );
            return values;
        } catch (e) {
            throw new Error(
                'Error getting organization metadata values',
                Date.now() - start
            );
        }
    }
    ...
    async setOrganizationMetadataValues(orgId, values, activeStatus = false) {
        ...
        logger.debug(
            `setOrganizationMetadataValues Attempting to update organization metadata`,
            Date.now() - start
        );
        ...
        const existingValues = await this.getOrganizationMetadataValues(
            orgId,
            keyList,
            activeStatus
        );
        ...
        logger.debug(
            `setOrganizationMetadataValues existingValues: ${JSON.stringify(existingValues)} validKeys: ${validKeys.join(', ')} keyList: ${keyList.join(', ')} updateKeyList: ${updateKeyList.join(', ')} newKeyList: ${newKeyList.join(', ')}`,
            Date.now() - start
        );
        ...
        if (newKeyList.length > 0) {
            ...
            sql +=
                'INSERT INTO raesum_organization_x_metadata (org_id, datakey, value) VALUES ';
        ...
        try {
            ...
            logger.error(
                `Failed setOrganizationMetadataValues to insert new matadata values for organization: ${e.message}`,
                Date.now() - start
            );
            throw new Error(
                'Error setOrganizationMetadataValues inserting new organization metadata values'
            );
        }
        ...
        for (let q = 0; q < updateKeyList.length; q++) {
            ...
            try {
                ...
                logger.error(
                    `Failed setOrganizationMetadataValues to update organization metadata values: ${e.message}`,
                    Date.now() - start
                );
                throw new Error(
                    'Error setOrganizationMetadataValues updating new organization metadata values'
                );
            }
        }

        logger.verbose(
            'Organization setOrganizationMetadataValues completed for org: ' +
                orgId +
                ' with keys ' +
    ...
    async deleteOrganizationMetadataValues(orgId, keys, activeStatus = false) {
        ...
        if (keysToDelete.length === 0) {
            logger.verbose(
                `No valid keys to delete for organizationId ${orgId}`,
                Date.now() - start
            );
            return true;
        }
        ...
        try {
            ...
            logger.info(
                `Deleted ${keysToDelete.length} metadata values for organization ${orgId}`,
                Date.now() - start
            );
        } catch (e) {
            logger.error(
                `Error deleting organization metadata values for organization ${orgId}: ${e.message}`,
                Date.now() - start
            );
