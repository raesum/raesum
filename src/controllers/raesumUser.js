    ...
    * The login function uses the cognito-hosted interface and will redirect the user to cognito's login page.
    ...
    async getMetaDataKeys(req,res,next){
        ...
        try{
            ...
            // Add audit log entry 
            await raesumAudit.create("read", "raesum_user", req.user.id, req.user.id);
            ...
            logger.error(`Error getting metadata keys: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async getAllUserMetaData(req,res,next){
        ...
        try{
            ...
            logger.error(`Error getting all user metadata for user ${userId}: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async getOneUserMetaData(req,res,next){
        ...
        logger.verbose(`getOneUserMetaData attempting to get key ${getKey} for user ${userId}`, Date.now() - start);
        ...
        if (typeof getKey != "string" || !validKeys.map(key => key.toLowerCase()).includes(getKey)) {
        ...
        try{
            // Get the metadata values for the user
            const metadata = await raesumUser.getUserMetadataValues(userId, [getKey]);
            
            logger.info(`User metadata retrieved for user ${userId} and key ${getKey}`, Date.now() - start);
            ...
            logger.error(`Error getting user metadata for user ${userId} and key ${req.params.key}: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async setOneUserMetaData(req,res,next){
        ...
        if (!req.params.key) {
            const message = await raesumResponses.get("requestMissingFields",["key"]);
            return res.status(message.code).json(message);
        }
        ...
        if (!value) {
            const message = await raesumResponses.get("requestMissingFields",["value"]);
            return res.status(message.code).json(message);
        }
        ...
        try{
            ...
            logger.info(`User metadata set for user ${userId}`, Date.now() - start);
            await raesumAudit.create("update", "raesum_user", userId, req.user.id);

        }catch(e){
            logger.error(`Error updating user metadata for user ${userId} and key ${req.params.key}: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async deleteOneUserMetaData(req,res,next){
        ...
        if (typeof deleteKey != "string" || !validKeys.map(key => key.toLowerCase()).includes(deleteKey)) {
            const message = await raesumResponses.get("requestInvalidFields",["key"]);
            return res.status(message.code).json(message);
        }
        ...
        try{
            ...
            logger.info(`User metadata deleted for user ${userId} and key ${deleteKey}`, Date.now() - start);

            // Add audit log entry 
            await raesumAudit.create("delete", "raesum_user_metadata", userId, req.user.id);
        
        }catch(e){
            logger.error(`Error deleting user metadata for user ${userId} and key ${req.params.key}: ${e.message}`);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async setUserActivation(req,res,next){
        ...
        try{
            ...
            logger.info(`User activation status updated for user ${userId}`, Date.now() - start);
            
            const message = await raesumResponses.get("success");
            return res.status(message.code).json(message);
        }
        catch(error){
            logger.error(`Error updating user activation status for user ${userId}: ${error.message}`, Date.now() - start);
            const message = await raesumResponses.get("error");
            return res.status(message.code).json(message);
        }
    }
    ...
    async changeUserOrg(req,res,next){
        ...
        if(!req.body.organizationId){
            const message = await raesumResponses.get("requestMissingFields",["organizationId"]);
            return res.status(message.code).json(message);
        }
        ...
        try{
            ...
            logger.info(`User organization changed for user ${userId}`, Date.now() - start);
            
            const message = await raesumResponses.get("success");
            return res.status(message.code).json(message);
        }
        catch(error){
            logger.error(`Error changing user organization for user ${userId}: ${error.message}`, Date.now() - start);
            const message = await raesumResponses.get("error");
            return res.status(message.code).json(message);
        }
    }
