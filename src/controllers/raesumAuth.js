...
class raesumAuthController {
    ...
    async login(req, res, next) {
        ...
        if (
            ...
            if (allowedCallbacksUpper.includes(raesumServerURL.toUpperCase())) {
                ...
                logger.warning(
                    'Login Redirect URI not allowed. Redirecting to the first allowed callback',
                    Date.now() - start
                );
                loginURL +=
    ...
    async logout(req, res, next) {
        ...
        try {
            ...
            const enableTokenRevocation =
                (await raesumConfig.get('aws.cognito.enableTokenRevocation')) ||
                true;
            ...
            try {
                req.session.destroy((err) => {
                    if (err) {
                        logger.error(
                            `Failed to destroy session for userID ${userId}: ${err.message}`,
                            Date.now() - start
                        );
                    } else {
                        logger.info(
                            'Successfully destroyed session for userID ' +
                                userId,
                            Date.now() - start
                        );
                    }
                ...
                logger.warning(
                    `Session destruction error for userID ${userId}: ${sessionError.message}`,
                    Date.now() - start
                );
            ...
            logger.error(
                `Logout process failed: ${error.message}`,
                Date.now() - start
            );
    ...
    async callbackSession(req, res, next) {
        const start = Date.now();
        logger.verbose(
            'Session login callback received, starting processing',
            Date.now() - start
        );
        // If the code param is not supplied, redirect to the login page
        if (!req.query.code) {
            logger.warning(
                'No code supplied. Redirecting to login',
                Date.now() - start
            );
            // Get the message
        ...
        if (loginMethods.useSessionCookie == false) {
            logger.verbose(
                'Session based logins not allowed',
                Date.now() - start
            );
        ...
        if (req.session.loginType == 'jwt') {
            ...
            logger.verbose(
                'JWT Login type is being used. Ignoring session login.',
                Date.now() - start
            );
        ...
        if (req.session.loggedIn == false || !req.session.userID) {
            logger.info(
                'User is not logged in, processing JWT and creating session'
            );
            ...
            logger.info(
                `User ${tokenPayload.sub} successfully logged in via session cookie`,
                Date.now() - start
            );
        } else {
            logger.info(
        ...
        if (req.session.post_login_uri) {
            console.verbose(
                'Post Login URI: ' + req.session.post_login_uri,
                Date.now() - start
            );
            ...
            if (isAbsoluteRegex.test(req.session.post_login_uri)) {
                try {
                    ...
                    let allowedDomains =
                        await raesumConfig.get('allowedOrigins');
                    ...
                    logger.error(
                        'Error parsing post_login_uri it seemed to be a full uri but failed to parse.',
                        Date.now() - start
                    );
                }
            } else {
    ...
    async callbackJWT(req, res, next) {
        ...
        try {
            ...
            if (!jwtResult.success) {
                logger.error(`Token exchange failed`, Date.now() - start);
            ...
            // Update user metadata from Cognito
            try {
                await raesumUser.syncUserFromCognitoToRaesum(tokenResponse.sub);
            } catch (error) {
                logger.warning(
                    `Failed to sync user from Cognito to Raesum: ${error.message}`,
                    Date.now() - start
    ...
    async getAvailableRoles(req, res, next) {
        ...
        try {
            ...
            // Return the roles
            logger.info(
                `Available roles for organization ${orgId} retrieved`,
                Date.now() - start
            );
            ...
            logger.error(
                `Error getting available roles for organization ${orgId}: ${e.message}`,
                Date.now() - start
            );
            const message = await raesumResponses.get('internalServerError');
            return res.status(message.code).json(message);
    ...
    async addUserRoles(req, res, next) {
        ...
        try {
            const validRoleIds = await raesumAuthorization.getRolesForOrg(
                orgId,
                true
            );
            const validRoles = await raesumAuthorization.getRolesByIDs(
                validRoleIds,
                true
            );
            ...
            for (const role of roles) {
                ...
                if (
                    isNaN(candidateRoleID) ||
                    candidateRoleID < 1 ||
                    !Number.isInteger(candidateRoleID)
                ) {
            ...
            logger.error(
                `Error getting valid roles: ${error.message}`,
                Date.now() - start
            );
            const message = await raesumResponses.get('internalServerError');
            return res.status(message.code).json(message);
        ...
        try {
            ...
            for (const roleId of roleIds) {
                await raesumAuthorization.addUserToRole(
                    userId,
                    parseInt(roleId),
                    orgId
                );
            ...
            // Return success
            logger.info(
                `Roles ${addedRoles.join(', ')} added to user ${userId}`,
                Date.now() - start
            );
            ...
            logger.error(
                `Error adding roles to user ${userId}: ${e.message}`,
                Date.now() - start
            );
            const message = await raesumResponses.get('internalServerError');
            return res.status(message.code).json(message);
    ...
    async deleteUserRoles(req, res, next) {
        ...
        const rolesByID = [];
        try {
            const userRoles = await raesumAuthorization.getUserRoles(
                userId,
                orgId
            );
            ...
            for (const role of req.body.values) {
                ...
                if (
                    isNaN(candidateRoleID) &&
                    typeof candidateRoleID === 'number' &&
                    userRoles.some((r) => r.id === candidateRoleID)
                ) {
                    rolesByID.push(candidateRoleID);
                } else if (
                    typeof role === 'string' &&
                    userRoles.some((r) => r.string_key === role)
                ) {
                    rolesByID.push(
                        userRoles.find((r) => r.string_key === role).id
                    );
                } else {
            ...
            logger.error(
                `Error getting validating the requested roles to delete: ${e.message}`,
                Date.now() - start
            );
            const message = await raesumResponses.get('internalServerError');
            return res.status(message.code).json(message);
        ...
        try {
            ...
            for (const roleId of rolesByID) {
                await raesumAuthorization.removeUserFromRole(
                    userId,
                    roleId,
                    orgId
                );
            ...
            // Return success
            logger.info(
                `Roles ${removedRoles.join(', ')} removed from user ${userId}`,
                Date.now() - start
            );
