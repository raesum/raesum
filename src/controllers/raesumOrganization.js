...
class raesumOrganizationController {
    ...
    async getById(req,res,next){
        ...
        if (!isAuthorized) {
            ...
            const message = await raesumResponses.get("notAuthorized",["read","raesum_organization"]);
            return res.status(message.code).json(message);
        }

        try{
            // Get the organization
            const organization = await raesumOrganization.getById(organizationId);
            
            // Return organization
            logger.info(`Organization ${organizationId} retrieved`, Date.now() - start);

            // Add audit log entry 
            await raesumAudit.create("read", "raesum_organization", organizationId, req.user.id); 
            const message = await raesumResponses.get("success");
            message.data = organization;
            return res.status(message.code).json(message);

        }catch(e){
            logger.error(`Error getting organization ${organizationId}: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async getMetaDataKeys(req,res,next){
        ...
        if (!isAuthorized) {
            // If not, get the not authorized message and return the rejected request
            const message = await raesumResponses.get("notAuthorized",["read","raesum_organization_metadata"]);
            return res.status(message.code).json(message);
        }
        ...
        try{
            ...
            logger.info(`Organization metadata keys retrieved`, Date.now() - start);

            // Add audit log entry 
            await raesumAudit.create("read", "raesum_organization_metadata", null, req.user.id); 
            const message = await raesumResponses.get("success");
            message.data = metadataKeys;
            return res.status(message.code).json(message);

        }catch(e){
            logger.error(`Error getting organization metadata keys: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async getOneOrganizationMetaData(req,res,next){
        ...
        logger.verbose(`getOneOrganizationMetaData attempting to get key ${getKey} for organization ${organizationId}`, Date.now() - start);
        ...
        if (typeof getKey != "string" || !validKeys.includes(getKey)) {
            const message = await raesumResponses.get("requestInvalidFields",["key"]);
            return res.status(message.code).json(message);
        }
        ...
        try{
            ...
            const metadata = await raesumOrganization.getOrganizationMetadataValues(organizationId, [getKey]);
            
            logger.info(`Organization metadata retrieved for organization ${organizationId} and key ${getKey}`, Date.now() - start);

            // Add audit log entry 
            await raesumAudit.create("read", "raesum_organization_metadata", organizationId, req.user.id); 
            const message = await raesumResponses.get("success");
            message.data = metadata;
            return res.status(message.code).json(message); 

        }catch(e){
            logger.error(`Error getting organization metadata for organization ${organizationId} and key ${req.params.key}: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async getAllOrganizationMetaData(req,res,next){
        ...
        try{
            ...
            const metadata = await raesumOrganization.getOrganizationMetadataValues(organizationId, keyList);
            
            logger.info(`Organization metadata retrieved for organization ${organizationId}`, Date.now() - start);
            ...
            logger.error(`Error getting organization metadata for organization ${organizationId}: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async setOneOrganizationMetaData(req,res,next){
        ...
        logger.verbose(`setOneOrganizationMetaData attempting to set key ${setKey} for organization ${organizationId}`, Date.now() - start);
        ...
        if (!isAuthorized) {
            // If not, get the not authorized message and return the rejected request
            const message = await raesumResponses.get("notAuthorized",["write","raesum_organization_metadata"]);
            return res.status(message.code).json(message);
        }
        ...
        try{
            ...
            if (typeof setValue != "string" && typeof setValue != "boolean" && typeof setValue != "number") {
                const message = await raesumResponses.get("requestInvalidFields",["value"]);
                return res.status(message.code).json(message);
            }

            // Set the metadata value for the organization
            await raesumOrganization.setOrganizationMetadataValues(organizationId, {[setKey]: setValue});
            
            logger.info(`Organization metadata set for organization ${organizationId} and key ${setKey}`, Date.now() - start);

            // Add audit log entry 
            await raesumAudit.create("write", "raesum_organization_metadata", organizationId, req.user.id); 
            const message = await raesumResponses.get("success");
            return res.status(message.code).json(message); 

        }catch(e){
            logger.error(`Error setting organization metadata for organization ${organizationId} and key ${req.params.key}: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async deleteOneOrganizationMetaData(req,res,next){
        ...
        logger.verbose(`deleteOneOrganizationMetaData attempting to delete key ${deleteKey} for organization ${organizationId}`, Date.now() - start);
        ...
        if (!isAuthorized) {
            // If not, get the not authorized message and return the rejected request
            const message = await raesumResponses.get("notAuthorized",["delete","raesum_organization_metadata"]);
            return res.status(message.code).json(message);
        }
        ...
        try{
            ...
            await raesumOrganization.deleteOrganizationMetadataValues(organizationId, [deleteKey]);
            
            logger.info(`Organization metadata deleted for organization ${organizationId} and key ${deleteKey}`, Date.now() - start);

            // Add audit log entry 
            await raesumAudit.create("delete", "raesum_organization_metadata", organizationId, req.user.id); 
            const message = await raesumResponses.get("success");
            return res.status(message.code).json(message); 

        }catch(e){
            logger.error(`Error deleting organization metadata for organization ${organizationId} and key ${req.params.key}: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
    ...
    async setOrganizationActivation(req,res,next){
        ...
        if (typeof activationStatus !== 'boolean') {
            if(activationStatus !== undefined && activationStatus.toLowerCase() == "true"){
        ...
        logger.debug(`Setting activation status for organization with ID: ${organizationId} to: ${activationStatus}`, Date.now() - start);
        ...
        if (!isAuthorized) {
            // If not, get the not authorized message and return the rejected request
            const message = await raesumResponses.get("notAuthorized",["write","raesum_organization"]);
            return res.status(message.code).json(message);
        }

        try{
            // Set the activation status
            const result = await raesumOrganization.setActivationStatus(organizationId, activationStatus);
            
            logger.info(`Activation status set for organization with ID: ${organizationId} to: ${activationStatus}`, Date.now() - start);

            // Add audit log entry 
            await raesumAudit.create("write", "raesum_organization", organizationId, req.user.id); 
            const message = await raesumResponses.get("success");
            message.data = {activationStatus: result};
            return res.status(message.code).json(message); 

        }catch(e){
            logger.error(`Error setting activation status for organization ${organizationId}: ${e.message}`, Date.now()-start);
            const message = await raesumResponses.get("internalServerError");
            return res.status(message.code).json(message);
        }
