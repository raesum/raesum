...
class raesumRateLimiter {
    ...
    async buildRateLimiter() {
        ...
        let blockDurationMinutes = parseInt(
            rateLimitConfig.blockDurationMinutes
        );
        ...
        if (rateLimitConfig.type != 'redis') {
            logger.warning(
                'RateLimiter is not using redis, using memory limiter instead. NOT recommended for production.'
            );
            this.limiterInstance = rateLimiterMemory;
        }
        ...
        if (redisConfig == undefined || !redisConfig.host) {
            logger.error(
                'Redis Configuration not set for RateLimiter, falling back to memory store'
            );
        } else {
            try {
                ...
                logger.error(
                    `Failed to create Redis RateLimiter store: ${redisError.message}, falling back to memory store`
                );
                this.limiterInstance = rateLimiterMemory;
            }
        }
    ...
    async init() {
        ...
        if (typeof this.limiterInstance == 'undefined') {
            ...
            logger.debug(
                'Rate Limiter already initialized',
                Date.now() - start
            );
            return true;
        }
