...
class raesumMigrate {
    ...
    async getCurrentSchemaVesion() {
        ...
        const databaseName = await raesumConfig.get(
            'connections.primaryDatabase.database'
        );

        const query1 =
            'SELECT EXISTS (SELECT FROM information_schema.tables WHERE  table_catalog = $1 AND table_name  = $2) exists;';
        ...
        const query2 =
            "SELECT datavalue FROM raesum_metadata WHERE datakey='schemaVersion' LIMIT 1;";
    ...
    async loadUpdateStaticContent(filename, targetTable) {
        ...
        logger.info(
            'Loading static content from ' + filename + ' into ' + targetTable,
            Date.now() - start
        );
        ...
        for (let i = 0; i < jsonData.length; i++) {
            ...
            if (Object.keys(jsonData[i]).length > 0) {
                ...
                const insertStatement =
                    ...
                    primaryKey +
                    ') DO UPDATE SET ' +
                    fields.map((el) => el + ' = EXCLUDED.' + el).join(',');
                ...
                try {
                    ...
                    // Halt on errors
                    logger.critical(
                        'Error loading static content into ' +
                            targetTable +
                            ' with error: ' +
                            e,
                        Date.now() - start
    ...
    async loadAllStaticContent() {
        ...
        const staticContent = [
            {
                filename: 'authorization/action.json',
                targetTable: 'raesum_auth_action_type',
            },
            {
                filename: 'authorization/object.json',
                targetTable: 'raesum_auth_object_type',
            },
            {
                filename: 'authorization/scope.json',
                targetTable: 'raesum_auth_scope_type',
            },
            {
                filename: 'file/fileStatus.json',
                targetTable: 'raesum_file_status',
            },
            {
        ...
        for (let i = 0; i < staticContent.length; i++) {
            // Run load function
            let loadReturn = await this.loadUpdateStaticContent(
                staticContent[i].filename,
                staticContent[i].targetTable
            );
    ...
    async doMigration() {
        ...
        if (schemaVersion > 0) {
            try {
                let migrationInProgress = await raseumMetadata.getByKey(
                    'migrationInProgress'
                );
                if (migrationInProgress == 1) {
                    logger.error(
                        'Update in progress, cannot run migration',
                        Date.now() - start
                    );
                    return false;
                } else {
                ...
                logger.error(
                    'No metadata key migrationInProgress found but SHOULD be present. Aborting migration.',
                    Date.now() - start
                );
                return false;
            }
            ...
            logger.info(
                'No update in progress, proceeding with initial migration',
                Date.now() - start
            );
        }
        ...
        logger.info(
            migrationsToRun.length + ' migrations to run',
            Date.now() - start
        );
        ...
        for (let i = 0; i < migrationsToRun.length; i++) {
            logger.info(
                'Running migration ' + migrationsToRun[i],
                Date.now() - start
            );

            // Load the SQL file
            const sqlFile = fs.readFileSync(
                path.join(dirPath, migrationsToRun[i]),
                'utf8'
            );
            ...
            const query =
                "UPDATE raesum_metadata SET datavalue = $1 WHERE datakey = 'schemaVersion';";
