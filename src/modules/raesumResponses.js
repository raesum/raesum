...
class raesumResponses {
    ...
    async init() {
        ...
        if (
            ...
            Object.keys(this.#responseCache).length === 0
        ) {
            ...
            try {
                ...
                const responseJSON = JSON.parse(
                    fs.readFileSync(
                        path.join(dirPath, 'responses.json'),
                        'utf8'
                    )
                );
            ...
            logger.info(
                'Finished Initializing raesumResponses',
                Date.now() - start
            );
        }
    }
    ...
    async get(responseKey, variables = []) {
        ...
        if (Object.hasOwn(this.#responseCache, responseKey)) {
            ...
            if (variables.length > 0 && variableCount == variables.length) {
                logger.debug(
                    'Replacing variables in response: ' + responseKey,
                    Date.now() - start
                );
                ...
                logger.error(
                    'Variable count does not match: ' + responseKey,
                    Date.now() - start
                );
            ...
            logger.error(
                'Response key not found: ' + responseKey,
                Date.now() - start
            );
