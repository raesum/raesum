...
class raesumSeed {
    async canSeed() {
        ...
        logger.info(
            'Checking to see if database can be seeded',
            Date.now() - start
        );
        ...
        logger.verbose(
            'Most Recent Migration Version: ' + numberElement,
            Date.now() - start
        );

        if (numberElement > schemaVersion) {
            logger.error(
                'Database is not current. Please run the migration script before seeding the database.',
                Date.now() - start
            );
            return false;
        }
        ...
        try {
            if (isProductionDatabase) {
                logger.critical(
                    'Database is a production database. Production databases CANNOT be seeded.',
                    Date.now() - start
                );
                return false;
            }
            ...
            logger.info(
                'Database has not yet been tattooed as a production or non-production database. Proceeding with seeding.',
                Date.now() - start
            );
        }
    ...
    async seedDB() {
        ...
        try {
            ...
            logger.info(
                'Database has not yet been tattoed as a production or non-production database. Proceeding with seeding.',
                Date.now() - start
            );
        }
        ...
        const createInCognito = await raesumConfig.get(
            'developmentAndTesting.seed.createInCognito'
        );
        if (createInCognito) {
        ...
        const seedScale = await raesumConfig.get(
            'developmentAndTesting.seed.scaleFactor'
        );
        ...
        logger.info(
            'Creating Seed Organizations: Client Type',
            Date.now() - start
        );
        for (let i = 0; i < clientTypeOrgCount; i++) {
            ...
            userCount += await this.#createSeedUsersForOrg(
                orgID,
                orgType,
                firstUserID,
                []
        ...
        logger.info(
            'Created ' + clientTypeOrgCount + ' Client Type Organizations',
            Date.now() - start
        );

        // Create 'Agency' Type Organizations
        logger.info(
            'Creating Seed Organizations: Agency Type',
            Date.now() - start
        );
        ...
        for (let i = 0; i < agencyTypeOrgCount; i++) {
            ...
            for (let c = 0; c < numberOfClients; c++) {
                ...
                const clientIndex = Math.floor(
                    Math.random() * clientTypeOrgList.length
                );
            ...
            logger.verbose(
                `Created the client list for org: ${orgID} with ${clientList.length} clients. Expected number of clients: ${numberOfClients}`
            );
        ...
        logger.info(
            'Created ' + agencyTypeOrgCount + ' Agency Type Organizations',
            Date.now() - start
        );

        // Randomly Deactivate a client organization
        const deactivateIndex = Math.floor(
            Math.random() * clientTypeOrgList.length
        );
        ...
        logger.info(
            `Created ${orgCount} Organizations and ${userCount} Users`,
            Date.now() - start
        );
    ...
    async #createUserMetadataKeys() {
        ...
        for (let i = 0; i < userMetadataKeys.length; i++) {
            ...
            try {
                await raesumUser.setUserMetadataKey(
                    key,
        ...
        logger.info(
            `Created ${userMetadataKeys.length} User metadata keys`,
            Date.now() - start
        );
    }
    ...
    async #createSeedUsersForOrg(orgID, orgType, firstUserID, clientOrgs = []) {
        ...
        const orgAdmin = await raesumConfig.get(
            'developmentAndTesting.seed.seedRoles.orgAdmin'
        );
        const orgManager = await raesumConfig.get(
            'developmentAndTesting.seed.seedRoles.orgManager'
        );
        const baseUser = await raesumConfig.get(
            'developmentAndTesting.seed.seedRoles.baseUser'
        );
        ...
        for (let i = 0; i < userCount; i++) {
            ...
            const userID = await raesumUser.createUser(
                external_id,
                userName + '_' + orgID + '_' + i,
                orgID,
                true
            );
        ...
        if (orgType == 'agency') {
            logger.verbose(
                `Org: ${orgID} is an 'agency' type. Adding agency users to client ${clientOrgs.length} orgs`,
                Date.now() - start
            );
            ...
            logger.verbose(
                `Org: ${orgID} will have ${clientAdminCount.length} client administrators for ${clientOrgs.length} clients`,
                Date.now() - start
            );
        ...
        logger.info(
            `Generating Audit Logs for Users in Org: ${orgID}`,
            Date.now() - start
        );
        ...
        logger.info(
            `Finished Generating Audit Logs for Users in Org: ${orgID}`,
            Date.now() - start
        );
    ...
    async #generateAuditLogs(userID) {
        ...
        const orgUsers = await raesumOrganization.getUsers(
        ...
        const sessionDates = faker.date.betweens({
            from: oldestDate,
            to: newestDate,
            count: sessionCount,
        });
        ...
        for (let s = 0; s < sessionCount; s++) {
            ...
            const sessionEnd = new Date(
                sessionDates[s].getTime + 60 * 60 * 24 * 1000
            );
            ...
            const currentSessionDate = faker.date.betweens({
                from: sessionDates[s],
                to: sessionEnd,
                count: eventCount,
            });
            ...
            let query =
                'BEGIN; INSERT INTO raesum_audit_log (action_id, object_id, object_type_id, user_id, event_at) VALUES \n';
            let valueArray = [];
                ...
                if (e == 0) {
                    ...
                    switch (eventID) {
                        ...
                        case 3:
                            ...
                            const roleIndex = Math.abs(
                                Math.floor(Math.random() * userRoles.length - 1)
                            );
            ...
            try {
                ...
                logger.verbose(
                    `Seed User: ${userID} Audit Logs Generated`,
                    Date.now() - start
                );
                return true;
            } catch (e) {
                logger.error(
                    `Seed User: ${userID} Audit Logs Failed`,
                    Date.now() - start
                );
    ...
    async #truncateRaesumTables() {
        ...
        for (let i = 0; i < tableList.length; i++) {
            const query =
                'TRUNCATE TABLE ' + tableList[i] + ' RESTART IDENTITY CASCADE;';
            try {
                await raesumDB.query(query);
            } catch (e) {
                logger.error(
                    `Error truncating table ${tableList[i]} with error: ${e}`,
                    Date.now() - start
                );
                return false;
            }
    ...
    async #loadSQLSeed(filename) {
        ...
        try {
            ...
            logger.error(
                'Error loading SQL Seed ' + filename + ' with error: ' + e,
                Date.now() - start
            );
            return false;
        }
