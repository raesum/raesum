...
class raesumCognito {
    ...
    async buildBaseLoginURL() {
        ...
        if (userPoolDescription.UserPool.CustomDomain) {
            logger.debug(
                'Using custom domain for Cognito login',
                Date.now() - start
            );
            ...
            logger.debug(
                'Using AWS hosted domain for Cognito login',
                Date.now() - start
            );
            ...
            loginURL +=
                userPoolDescription.UserPool.Domain +
                '.auth.' +
                region +
                '.amazoncognito.com';
        }
    ...
    async getCognitoUserPoolDescription() {
        ...
        if (cacheValue) {
            logger.debug(
                'Cognito user pool information found in cache',
                Date.now() - start
            );
            return cacheValue;
        }
        ...
        logger.info(
            'Cognito user pool information not in cache. Retrieving from AWS',
            Date.now() - start
        );
        ...
        logger.verbose(
            'Getting Cognito User Pool Description',
            Date.now() - start
        );
        ...
        try {
            ...
            logger.verbose(
                `Cognito User Pool Description Retrieved`,
                Date.now() - start
            );
            return response;
        } catch (e) {
            logger.error(
                `Error getting Cognito User Pool Description: ${e}`,
                Date.now() - start
            );
            throw new Error(e);
        }
    ...
    async getCognitoClientDescription(cacheBust = false) {
        ...
        // Initialize the cognito client
        logger.info(
            'Cognito client information not in cache. Retrieving from AWS',
            Date.now() - start
        );
        let awsCognitoConfig = await buildAWSClientConfig();
        ...
        logger.verbose(
            'Getting Cognito Client Description',
            Date.now() - start
        );
        const client = new CognitoIdentityProviderClient(awsCognitoConfig);
        const input = {
        ...
        try {
            ...
            logger.error(
                `Error getting Cognito Client Description: ${e}`,
                Date.now() - start
            );
            throw new Error(e);
        }
    ...
    async getAllowedCallbacks() {
        ...
        if (
            ...
            logger.error(
                'Unable to get Cognito Client Description',
                Date.now() - start
            );
            return [];
        } else {
    ...
    async synchronizeCognitoUserMetadata() {
        ...
        logger.info(
            'Checking Cognito Connection and updating user metadata keys',
            Date.now() - start
        );
        try {
        ...
        for (let i in attrList) {
            ...
            if (metadataKeys.includes(i)) {
                ...
                sql +=
                    'UPDATE raesum_user_metadata_keys SET cognito_attribute = true , cognito_writable = ' +
                    attrList[i] +
                    " WHERE datakey ILIKE '" +
                    i +
                    "';\n";
                ...
                sql +=
                    "INSERT INTO raesum_user_metadata_keys (datakey, cognito_attribute, cognito_writable, active_status) VALUES ('" +
                    i +
                    "', true, " +
                    attrList[i] +
                    ', true);\n';
        ...
        logger.info(
            'Cognito connection successful and user metadata keys updated',
            Date.now() - start
        );
        return true;
    }
    ...
    async exchangeCodeForTokens(code, redirectUri) {
        ...
        try {
            ...
            logger.info(
                'Successfully exchanged authorization code for JWT tokens',
                Date.now() - start
            );
    ...
    async processJWT(req) {
        ...
        try {
            ...
            logger.verbose(
                'Is the current server URL allowed in the callbacks',
                Date.now() - start
            );
            ...
            if (!allowedCallbacks.includes(raesumServerURL)) {
                logger.error(
                    'No allowed callbacks found for token exchange',
                    Date.now() - start
                );
                return {
                    success: false,
                    responseMessageKey: 'internalServerError',
                    user: null,
                    jwt: null,
                };
            }
            ...
            logger.verbose(
                'Validating ID token to get user information',
                Date.now() - start
            );

            const tokenPayload = await this.validateJWTToken(
            ...
            try {
                ...
                logger.info(
                    `User ${cognitoUserId} does not exist in Raesum DB`,
                    Date.now() - start
                );
            }
            ...
            if (user && user.active_status === false) {
                ...
                return {
                    success: false,
                    responseMessageKey: 'invalidUserPool',
                    user: null,
                    tokenResponse: null,
                    tokenPayload: null,
                };
            ...
            if (!user) {
                logger.verbose(
                    "User doesn't exist in Raesum DB, starting new user creation",
                    Date.now() - start
                );
                try {
                    ...
                    logger.verbose(
                        'New User Defaults: ' + JSON.stringify(newUserDefaults),
                        Date.now() - start
                    );
                    const newUser = await raesumUser.createUser(
                        cognitoUserId,
                        username,
                    ...
                    logger.info(
                        `Created new user in Raesum DB for Cognito user ${newUser}`,
                        Date.now() - start
                    );
                    ...
                    logger.error(
                        `Failed to create user in Raesum DB: ${createError.message}`,
                        Date.now() - start
                    );
                    return {
                        success: false,
                        responseMessageKey: 'unableToCreateEditUser',
                        user: null,
                        tokenResponse: null,
                        tokenPayload: null,
                    };
            ...
            logger.error(
                'Error exchanging code for JWT tokens',
                Date.now() - start
            );
            return {
                success: false,
    ...
    async globalSignOut(accessToken) {
        ...
        try {
            // Initialize the cognito client
            logger.verbose(
                'Initializing awsCognito client for global sign out',
                Date.now() - start
            );
            ...
            // Execute the command
            logger.verbose(
                'Sending client command for global sign out',
                Date.now() - start
            );
            ...
            logger.verbose(
                'Adding JWT to internal cached revokelist',
                Date.now() - start
            );
            try {
                ...
                if (enableTokenRevocation) {
                    ...
                    logger.verbose(
                        `Using token remaining time for cache TTL: ${cacheTTL} seconds (AWS revocation enabled)`,
                        Date.now() - start
                    );
                } else {
                    ...
                    logger.verbose(
                        `Using 1 year cache TTL: ${cacheTTL} seconds (AWS revocation disabled)`,
                        Date.now() - start
                    );
                }
                ...
                logger.info(
                    `Added JWT to revoked list in cache with TTL: ${cacheTTL} seconds`,
                    Date.now() - start
                );
            } catch (cacheError) {
                logger.warning(
                    `Failed to add JWT to revoked cache: ${cacheError.message}`,
                    Date.now() - start
                );
            ...
            logger.info(
                'Successfully performed global sign out in Cognito',
                Date.now() - start
            );
            return true;
        } catch (error) {
            logger.error(
                `Failed to perform global sign out: ${error.message}`,
                Date.now() - start
            );
            throw new Error(
                `Failed to perform global sign out: ${error.message}`
    ...
    async setUserEnabledStatus(username, enabled) {
        ...
        try {
            // Initialize the cognito client
            logger.verbose(
                `Initializing AWS Cognito client to ${enabled ? 'enable' : 'disable'} user: ${username}`,
                Date.now() - start
            );
            const awsCognitoConfig = await buildAWSClientConfig();
            const client = new CognitoIdentityProviderClient(awsCognitoConfig);
            ...
            // Execute the command
            logger.verbose(
                `Sending command to ${enabled ? 'enable' : 'disable'} Cognito user: ${username}`,
                Date.now() - start
            );
            await client.send(command);

            logger.info(
                `Successfully ${enabled ? 'enabled' : 'disabled'} Cognito user: ${username}`,
                Date.now() - start
            );
            return true;
        } catch (error) {
            logger.error(
                `Failed to ${enabled ? 'enable' : 'disable'} Cognito user ${username}: ${error.message}`,
                Date.now() - start
            );
            throw new Error(
                `Failed to ${enabled ? 'enable' : 'disable'} Cognito user: ${error.message}`
            );
        }
    }
    ...
    async createCognitoUser(
        ...
        // Initialize the cognito client
        logger.verbose(
            `Initializing AWS Cognito client for user: ${username}`,
            Date.now() - start
        );
        const awsCognitoConfig = await buildAWSClientConfig();
        const client = new CognitoIdentityProviderClient(awsCognitoConfig);
        ...
        try {
            logger.verbose(
                `Checking if user ${username} already exists in Cognito`,
                Date.now() - start
            );
            ...
            const existingUserId = existingUserResponse.UserAttributes?.find(
                (attr) => attr.Name === 'sub'
            )?.Value;

            if (existingUserId) {
                logger.info(
                    `User ${username} already exists in Cognito with ID: ${existingUserId}`,
                    Date.now() - start
                );
            ...
            if (getUserError.name === 'UserNotFoundException') {
                logger.verbose(
                    `User ${username} not found in Cognito, proceeding to create`,
                    Date.now() - start
                );
            } else {
                // Other errors when checking user - log but don't throw, try to create anyway
                logger.info(
                    `Error checking if user ${username} exists: ${getUserError.message}`,
                    Date.now() - start
                );
            }
        }
        ...
        try {
            ...
            logger.verbose(
                `Sending AdminCreateUser command for user: ${username}`,
                Date.now() - start
            );
            const response = await client.send(createCommand);

            // Extract the Cognito user ID (sub) from the response
            const cognitoUserId = response.User?.Attributes?.find(
                (attr) => attr.Name === 'sub'
            )?.Value;

            if (!cognitoUserId) {
                throw new Error(
                    'Cognito user created but no sub attribute found in response'
                );
            }

            logger.info(
                `Successfully created Cognito user: ${username} with ID: ${cognitoUserId}`,
                Date.now() - start
            );
            ...
            if (createError.name === 'UsernameExistsException') {
                logger.verbose(
                    `User ${username} already exists (race condition), fetching existing user`,
                    Date.now() - start
                );
                try {
                    const getUserCommand = new AdminGetUserCommand({
                    ...
                    const existingUserId =
                        existingUserResponse.UserAttributes?.find(
                            (attr) => attr.Name === 'sub'
                        )?.Value;

                    if (existingUserId) {
                        logger.info(
                            `Retrieved existing Cognito user: ${username} with ID: ${existingUserId}`,
                            Date.now() - start
                        );
                        return existingUserId;
                    } else {
                        throw new Error(
                            'User exists but no sub attribute found'
                        );
                    }
                } catch (finalError) {
                    logger.error(
                        `Failed to retrieve existing user ${username}: ${finalError.message}`,
                        Date.now() - start
                    );
                    throw new Error(
                        `User exists but unable to retrieve: ${finalError.message}`
                    );
                }
            }

            logger.error(
                `Failed to create Cognito user ${username}: ${createError.message}`,
                Date.now() - start
            );
            throw new Error(
                `Failed to create Cognito user: ${createError.message}`
            );
        }
    }
    ...
    getTokenExpiration(token) {
        try {
            ...
            logger.error(
                `Failed to decode token for expiration: ${error.message}`
            );
    ...
    async getCognitoUser(username) {
        ...
        try {
            ...
            logger.info(
                `Successfully retrieved Cognito user: ${username}`,
                Date.now() - start
            );
            return response;
        } catch (error) {
            logger.error(
                `Failed to get Cognito user ${username}: ${error.message}`,
                Date.now() - start
            );
    ...
    async updateCognitoUserAttributes(username, attributes) {
        ...
        try {
            // Initialize the cognito client
            logger.verbose(
                `Updating Cognito user attributes for: ${username}`,
                Date.now() - start
            );
            const awsCognitoConfig = await buildAWSClientConfig();
            const client = new CognitoIdentityProviderClient(awsCognitoConfig);
            ...
            logger.info(
                `Successfully updated ${attributes.length} Cognito user attributes for: ${username}`,
                Date.now() - start
            );
            return true;
        } catch (error) {
            logger.error(
                `Failed to update Cognito user attributes for ${username}: ${error.message}`,
                Date.now() - start
            );
            throw new Error(
                `Failed to update Cognito user attributes: ${error.message}`
            );
        }
    }
    ...
    async deleteCognitoUserAttributes(username, attributeNames) {
        ...
        try {
            // Initialize the cognito client
            logger.verbose(
                `Deleting Cognito user attributes for: ${username}`,
                Date.now() - start
            );
            const awsCognitoConfig = await buildAWSClientConfig();
            const client = new CognitoIdentityProviderClient(awsCognitoConfig);
            ...
            logger.info(
                `Successfully deleted ${attributeNames.length} Cognito user attributes for: ${username}`,
                Date.now() - start
            );
            return true;
        } catch (error) {
            logger.error(
                `Failed to delete Cognito user attributes for ${username}: ${error.message}`,
                Date.now() - start
            );
            throw new Error(
                `Failed to delete Cognito user attributes: ${error.message}`
            );
        }
    }
}
