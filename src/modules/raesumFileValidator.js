...
class raesumFileValidator {
    ...
    */
    async validateSize(fileSize, fileTypeKey){
        ...
        logger.debug(`Validating file size for type: ${fileTypeKey}`, Date.now() - start);

        try {
            // Get the file type details
            const fileTypeDetails = await raesumFile.getOneFileType(fileTypeKey);
            if (!fileTypeDetails) {
            ...
            if (!fileTypeDetails.fileSizeLimit) {
                logger.debug(`No file size limit defined for type: ${fileTypeKey}`, Date.now() - start);
                return true;
            }

            // Validate the file size
            const isValid = fileSize <= fileTypeDetails.fileSizeLimit;
            logger.debug(`File size validation result: ${isValid} (size: ${fileSize}, limit: ${fileTypeDetails.fileSizeLimit})`, Date.now() - start);
            return isValid;

        } catch (e) {
            logger.error(`Error validating file size for type ${fileTypeKey}: ${e.message}`, Date.now() - start);
            return false;
        }
    }
    ...
    async validateDimensions(fileWidth, fileHeight, fileTypeKey){
        const start = Date.now();
        logger.debug(`Validating file dimensions for type: ${fileTypeKey}`, Date.now() - start);
        ...
        try {
            ...
            if (!fileTypeDetails.dimensionsLimit || !fileTypeDetails.dimensionsLimit.width || !fileTypeDetails.dimensionsLimit.height) {
                logger.debug(`No dimension limit defined for type: ${fileTypeKey}`, Date.now() - start);
                return true;
            }

            // Validate the dimensions
            const isWidthValid = fileWidth <= fileTypeDetails.dimensionsLimit.width;
            const isHeightValid = fileHeight <= fileTypeDetails.dimensionsLimit.height;
            const isValid = isWidthValid && isHeightValid;

            logger.debug(`Dimension validation result: ${isValid} (width: ${fileWidth}, limit: ${fileTypeDetails.dimensionsLimit.width}, height: ${fileHeight}, limit: ${fileTypeDetails.dimensionsLimit.height})`, Date.now() - start);
            return isValid;

        } catch (e) {
            logger.error(`Error validating file dimensions for type ${fileTypeKey}: ${e.message}`, Date.now() - start);
            return false;
        }
    }
    ...
    async validateMime(fileMime, fileTypeKey){
        const start = Date.now();
        logger.debug(`Validating file mime type for type: ${fileTypeKey}`, Date.now() - start);
        ...
        try {
            ...
            if (!fileTypeDetails.mimeTypes || !Array.isArray(fileTypeDetails.mimeTypes) || fileTypeDetails.mimeTypes.length === 0) {
                logger.debug(`No mime types defined for type: ${fileTypeKey}`, Date.now() - start);
                return true;
            }

            // Validate the mime type (case-insensitive comparison)
            const isValid = fileTypeDetails.mimeTypes.some(allowedMime => allowedMime.toLowerCase() === fileMime.toLowerCase());
            logger.debug(`Mime type validation result: ${isValid} (mime: ${fileMime}, allowed: ${fileTypeDetails.mimeTypes.join(', ')})`, Date.now() - start);
            return isValid;

        } catch (e) {
            logger.error(`Error validating file mime type for type ${fileTypeKey}: ${e.message}`, Date.now() - start);
            return false;
        }
    }
    ...
    validateImageMime(fileMime){
        ...
        return knownImageFileTypes.includes(fileMime.toLowerCase());
    }
}
