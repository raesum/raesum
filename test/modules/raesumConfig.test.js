...
describe('Raesum Config raesumConfig.getCloudSecretKey', () => {
    test('Single Key Matching', async () => {
        ...
        expect(
            await raesumConfig.getCloudSecretKey(
                'connections.primaryDatabase.credentials'
            )
        ).toEqual(['connections.primaryDatabase.credentials']);
        expect(await raesumConfig.getCloudSecretKey('cows')).toEqual([]);
        expect(
            await raesumConfig.getCloudSecretKey('connections.primaryDatabase')
        ).toEqual(['connections.primaryDatabase.credentials']);
        expect(
            await raesumConfig.getCloudSecretKey(
                'connections.testDatabase.credentials'
            )
        ).toEqual(['connections.testDatabase']);
    });
...
describe('Raesum Config raesumConfigGet', () => {
    test('Single Key Non-cloud', async () => {
        ...
        expect(await raesumConfig.get('ephemeralKey')).toEqual(
            'ephemeralKeyValue'
        );
        expect(
            await raesumConfig.get('connections.primaryDatabase.credentials')
        ).toEqual({
            username: 'goat',
            password: 'frolics',
        });
    ...
    test('Child Keys in Cloud', async () => {
        const configMock = vi.spyOn(config, 'get').mockImplementation((key) => {
            ...
            if (key == 'cloudBasedSecrets') {
                ...
                key == 'connections.primaryDatabase.credentials.username'
            ) {
                returnVal = 'goat';
            } else if (
                key == 'connections.primaryDatabase.credentials.password'
            ) {
                returnVal = 'frolics';
