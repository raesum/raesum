import raesumAudit from "../../src/modules/raesumAudit.js";
...
describe("Raesum Audit System", () => {
    ...
    iftest('Add an audit log entry via IDs', async () => {
        ...
        const entryKey = await raesumAudit.logEvent(1, 1, 1, 1);
    ...
    iftest('Add an audit log entry via string_keys', async () => {
        ...
        // Set an audit log entry
        const entryKey = await raesumAudit.logEvent("log_in", "raesum_user", 1, 1);
    ...
    iftest('Add an audit log entry with invalid user', async () => {
        // Set an audit log entry
        expect(async ()=>{
            await raesumAudit.logEvent(1, 1, 1, -473);
        }).rejects.toThrow();
