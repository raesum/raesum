...
describe('Raesum User Model', () => {
    ...
    describe('createUser', () => {
        test('should create a user with valid parameters', async () => {
            ...
            const orgMock = vi
                ...
                .mockResolvedValue({ id: orgId, name: 'Test Org' });

            // Mock username uniqueness check
            const usernameMock = vi
                .spyOn(raesumUser, 'getUserByUsername')
                .mockRejectedValue(new Error('User not found'));

            // Mock external_id uniqueness check
            const externalIdMock = vi
                .spyOn(raesumUser, 'getUserByExternalID')
                .mockRejectedValue(new Error('User not found'));
            ...
            const addUserMock = vi
                .spyOn(raesumOrganization, 'addUserToOrganization')
                .mockResolvedValue(true);

            const result = await raesumUser.createUser(
                external_id,
                username,
                orgId,
                activeStatus
        ...
        test('should throw error for invalid external_id', async () => {
            await expect(
                raesumUser.createUser('', 'testuser', 1)
            ).rejects.toThrow('External ID must be a non-empty string');
            await expect(
                raesumUser.createUser(null, 'testuser', 1)
            ).rejects.toThrow('External ID must be a non-empty string');
            await expect(
                raesumUser.createUser(123, 'testuser', 1)
            ).rejects.toThrow('External ID must be a non-empty string');
        });

        test('should throw error for invalid username', async () => {
            await expect(
                raesumUser.createUser('ext123', '', 1)
            ).rejects.toThrow('Username must be a non-empty string');
            await expect(
                raesumUser.createUser('ext123', null, 1)
            ).rejects.toThrow('Username must be a non-empty string');
            await expect(
                raesumUser.createUser('ext123', 123, 1)
            ).rejects.toThrow('Username must be a non-empty string');
        });

        test('should throw error for invalid organization ID', async () => {
            await expect(
                raesumUser.createUser('ext123', 'testuser', 0)
            ).rejects.toThrow('Organization ID must be a positive integer');
            await expect(
                raesumUser.createUser('ext123', 'testuser', -1)
            ).rejects.toThrow('Organization ID must be a positive integer');
            await expect(
                raesumUser.createUser('ext123', 'testuser', 1.5)
            ).rejects.toThrow('Organization ID must be a positive integer');
            await expect(
                raesumUser.createUser('ext123', 'testuser', 'abc')
            ).rejects.toThrow('Organization ID must be a positive integer');
        });
        ...
        test('should throw error when username is not unique', async () => {
            ...
            vi.spyOn(raesumUser, 'getUserByUsername').mockResolvedValue({
                id: 123,
            });

            await expect(
                raesumUser.createUser('ext123', 'testuser', 1)
            ).rejects.toThrow('Username is not unique');
        });
        ...
        test('should throw error when external_id is not unique', async () => {
            ...
            vi.spyOn(raesumUser, 'getUserByUsername').mockRejectedValue(
            ...
            await expect(
                raesumUser.createUser('ext123', 'testuser', 1)
            ).rejects.toThrow('ExternalID is not unique');
        });
    });
    ...
    describe('getUserById', () => {
        test('should get user by valid ID', async () => {
            ...
            expect(dbMock).toHaveBeenCalledWith(
                'SELECT * FROM raesum_user WHERE id = $1',
                [userId]
            );
        });

        test('should throw error for invalid ID', async () => {
            await expect(raesumUser.getUserById(0)).rejects.toThrow(
                'User ID must be a positive integer'
            );
            await expect(raesumUser.getUserById(-1)).rejects.toThrow(
                'User ID must be a positive integer'
            );
            await expect(raesumUser.getUserById('abc')).rejects.toThrow(
                'User ID must be a positive integer'
            );
        });
        ...
        test('should throw error when user not found', async () => {
            ...
            await expect(raesumUser.getUserById(123)).rejects.toThrow(
                'User not found'
            );
        });
    ...
    describe('getUserByUsername', () => {
        test('should get user by valid username', async () => {
            ...
            expect(result).toBe(mockUser);
            expect(dbMock).toHaveBeenCalledWith(
                'SELECT id FROM raesum_user WHERE username = $1',
                [username]
            );
        ...
        test('should throw error for invalid username', async () => {
            await expect(raesumUser.getUserByUsername('')).rejects.toThrow(
                'Username must be a non-empty string'
            );
            await expect(raesumUser.getUserByUsername(null)).rejects.toThrow(
                'Username must be a non-empty string'
            );
            await expect(raesumUser.getUserByUsername(123)).rejects.toThrow(
                'Username must be a non-empty string'
            );
        });
        ...
        test('should throw error when user not found', async () => {
            ...
            await expect(
                raesumUser.getUserByUsername('nonexistent')
    ...
    describe('getUserByExternalID', () => {
        test('should get user by valid external ID', async () => {
            ...
            expect(result).toBe(mockUser);
            expect(dbMock).toHaveBeenCalledWith(
                'SELECT id FROM raesum_user WHERE external_id = $1',
                [externalId]
            );
            expect(getUserByIdMock).toHaveBeenCalledWith(userId);
        });
        ...
        test('should throw error for invalid external ID', async () => {
            await expect(raesumUser.getUserByExternalID('')).rejects.toThrow(
                'external_id must be a non-empty string'
            );
            await expect(raesumUser.getUserByExternalID(null)).rejects.toThrow(
                'external_id must be a non-empty string'
            );
            await expect(raesumUser.getUserByExternalID(123)).rejects.toThrow(
                'external_id must be a non-empty string'
            );
        });
        ...
        test('should throw error when user not found', async () => {
            ...
            await expect(
                raesumUser.getUserByExternalID('nonexistent')
            ).rejects.toThrow('Error getting user by external_id');
        });
    });
    ...
    describe('setActivationStatus', () => {
        test('should set activation status for valid user', async () => {
            ...
            const result = await raesumUser.setActivationStatus(
                userId,
                activeStatus
            );
        ...
        test('should rollback database change when Cognito update fails', async () => {
            ...
            await expect(
                raesumUser.setActivationStatus(userId, activeStatus)
            ).rejects.toThrow(
                'Failed to update user status in Cognito: Cognito error'
            );
        ...
        test('should throw error for invalid activeStatus', async () => {
            await expect(
                raesumUser.setActivationStatus(123, 'true')
            ).rejects.toThrow('Activation status must be a boolean');
            await expect(
                raesumUser.setActivationStatus(123, 1)
            ).rejects.toThrow('Activation status must be a boolean');
            await expect(
                raesumUser.setActivationStatus(123, null)
            ).rejects.toThrow('Activation status must be a boolean');
        });

        test('should throw error for invalid user ID', async () => {
            await expect(
                raesumUser.setActivationStatus(0, true)
            ).rejects.toThrow('User ID must be a positive integer');
            await expect(
                raesumUser.setActivationStatus(-1, true)
            ).rejects.toThrow('User ID must be a positive integer');
        });
    ...
    describe('getAllowedUserOrgs', () => {
        test('should get allowed organizations for valid user', async () => {
            const userId = 123;
            const mockOrgs = [{ org_id: 1 }, { org_id: 2 }];
        ...
        test('should throw error for invalid user ID', async () => {
            await expect(raesumUser.getAllowedUserOrgs(0)).rejects.toThrow(
                'User ID must be a positive integer'
            );
            await expect(raesumUser.getAllowedUserOrgs(-1)).rejects.toThrow(
                'User ID must be a positive integer'
            );
        });
        ...
        test('should throw error when database query fails', async () => {
            ...
            await expect(raesumUser.getAllowedUserOrgs(123)).rejects.toThrow(
                'Error getting allowed organizations'
            );
        });
    });
    ...
    describe('changeUserOrg', () => {
        test('should change user organization successfully', async () => {
            ...
            const getAllowedMock = vi
                .spyOn(raesumUser, 'getAllowedUserOrgs')
                .mockResolvedValue(allowedOrgs);
            const dbMock = vi.spyOn(raesumDB, 'query').mockResolvedValue({});
        ...
        test('should throw error for invalid user ID', async () => {
            await expect(raesumUser.changeUserOrg(0, 1)).rejects.toThrow(
                'User ID must be a positive integer'
            );
            await expect(raesumUser.changeUserOrg(-1, 1)).rejects.toThrow(
                'User ID must be a positive integer'
            );
        });

        test('should throw error for invalid organization ID', async () => {
            await expect(raesumUser.changeUserOrg(123, 0)).rejects.toThrow(
                'Organization ID must be a positive integer'
            );
            await expect(raesumUser.changeUserOrg(123, -1)).rejects.toThrow(
                'Organization ID must be a positive integer'
            );
        });

        test('should throw error when user not allowed to switch to organization', async () => {
            vi.spyOn(raesumUser, 'getAllowedUserOrgs').mockResolvedValue([
                1, 3,
            ]);

            await expect(raesumUser.changeUserOrg(123, 2)).rejects.toThrow(
                'User not allowed to switch to organization'
            );
        });
    ...
    describe('Metadata Methods', () => {
        describe('getMetadataKeys', () => {
            ...
            test('should get metadata keys from database when not cached', async () => {
                ...
                expect(dbMock).toHaveBeenCalledWith(
                    'SELECT * FROM raesum_user_metadata_keys WHERE active_status = true'
                );
                expect(cacheSetMock).toHaveBeenCalledWith(cacheKey, {
                    key1: { datakey: 'key1', id: 1 },
                    key2: { datakey: 'key2', id: 2 },
                });
            });
        });
        ...
        describe('setUserMetadataKey', () => {
            test('should create new metadata key', async () => {
                ...
                const result = await raesumUser.setUserMetadataKey(
                    key,
                    description
                );
            ...
            test('should throw error for invalid key characters', async () => {
                await expect(
                    raesumUser.setUserMetadataKey('invalid key')
            ...
            test('should throw error when database operation fails', async () => {
                ...
                await expect(
                    raesumUser.setUserMetadataKey('existingkey')
                ).rejects.toThrow('Error creating user metadata key');
            });
        });
        ...
        describe('deleteUserMetadataKey', () => {
            ...
            test('should throw error when key does not exist', async () => {
                ...
                await expect(
                    raesumUser.deleteUserMetadataKey('nonexistent')
        ...
        describe('getUserMetadataValues', () => {
            test('should get metadata values for valid user', async () => {
                ...
                const result = await raesumUser.getUserMetadataValues(
                    userId,
                    keys
                );
            ...
            test('should throw error for invalid user ID', async () => {
                await expect(
                    raesumUser.getUserMetadataValues(0, ['key1'])
                ).rejects.toThrow('User ID must be a positive integer');
                await expect(
                    raesumUser.getUserMetadataValues(-1, ['key1'])
                ).rejects.toThrow('User ID must be a positive integer');
            });
            ...
            test('should throw error when database query fails', async () => {
                ...
                await expect(
                    raesumUser.getUserMetadataValues(123, ['key1'])
                ).rejects.toThrow('Error getting user metadata values');
            });
        });
        ...
        describe('setUserMetadataValues', () => {
            test('should set metadata values for valid user', async () => {
                ...
                const result = await raesumUser.setUserMetadataValues(
                    userId,
                    values
                );
            ...
            test('should throw error when user does not exist', async () => {
                ...
                await expect(
                    raesumUser.setUserMetadataValues(123, { key1: 'value1' })
                ).rejects.toThrow('User not found');
            });
        ...
        describe('deleteUserMetadataValues', () => {
            test('should delete valid metadata values for user', async () => {
                ...
                const result = await raesumUser.deleteUserMetadataValues(
                    userId,
                    keys
                );
                ...
                expect(dbMock).toHaveBeenCalledWith(
                    expect.stringContaining(
                        'DELETE FROM raesum_user_x_metadata'
                    ),
                    [userId, ['key1', 'key2']]
                );
            });
            ...
            test('should delete Cognito writable attributes from both DB and Cognito', async () => {
                ...
                const keyStatus = {
                    cognitoKeys: keys,
                    readOnlyCognitoKeys: ['read_only_key'],
                };
                ...
                vi.spyOn(
                    raesumUser,
                    'getMetadataCognitoKeyStatus'
                ).mockResolvedValue(keyStatus);
                vi.spyOn(raesumDB, 'query').mockResolvedValue({});
                const cognitoDeleteMock = vi
                    .spyOn(raesumCognito, 'deleteCognitoUserAttributes')
                    .mockResolvedValue(true);
                ...
                expect(result).toBe(true);
                expect(cognitoDeleteMock).toHaveBeenCalledWith('testuser', [
                    'custom_key',
                ]);
            });
            ...
            test('should throw error for invalid user ID', async () => {
                await expect(
                    raesumUser.deleteUserMetadataValues(0, ['key1'])
                ).rejects.toThrow('User ID must be a positive integer');
                await expect(
                    raesumUser.deleteUserMetadataValues(-1, ['key1'])
                ).rejects.toThrow('User ID must be a positive integer');
                await expect(
                    raesumUser.deleteUserMetadataValues('abc', ['key1'])
                ).rejects.toThrow('User ID must be a positive integer');
            });
            ...
            test('should throw error when user does not exist', async () => {
                ...
                await expect(
                    raesumUser.deleteUserMetadataValues(123, ['key1'])
                ).rejects.toThrow('User not found');
            });
            ...
            test('should handle database error', async () => {
                ...
                await expect(
                    raesumUser.deleteUserMetadataValues(userId, keys)
                ).rejects.toThrow('Error deleting user metadata values');
            });
    ...
    describe('initRaesum', () => {
        test('should initialize raesum with default user', async () => {
            ...
            vi.spyOn(raesumConfig, 'get').mockImplementation((key) => {
                ...
                if (key === 'initialization.firstUserEmail')
                    return 'admin@test.com';
                return null;
            });

            vi.spyOn(raesumCognito, 'createCognitoUser').mockReturnValue(
                externalId
            );
        ...
        test('should default to organization ID 1 when not provided', async () => {
            const configMock = vi
                ...
                .mockImplementation((key) => {
                    if (key === 'initialization.firstUserUsername')
                        return 'default_user';
                    if (key === 'initialization.firstUserRole') return 'admin';
                    if (key === 'initialization.firstUserEmail')
                        return 'admin@test.com';
    ...
    describe('syncUserFromCognitoToRaesum', () => {
        ...
        test('should throw error if external_id is not a string', async () => {
            await expect(
                raesumUser.syncUserFromCognitoToRaesum(123)
            ).rejects.toThrow('External ID must be a non-empty string');
            await expect(
                raesumUser.syncUserFromCognitoToRaesum(null)
            ).rejects.toThrow('External ID must be a non-empty string');
            await expect(
                raesumUser.syncUserFromCognitoToRaesum('')
            ).rejects.toThrow('External ID must be a non-empty string');
        });

        test('should throw error if user not found in Cognito', async () => {
            vi.spyOn(raesumCognito, 'getCognitoUser').mockRejectedValue(
                new Error('User not found')
            );

            await expect(
                raesumUser.syncUserFromCognitoToRaesum(mockExternalId)
        ...
        test('should return existing user ID if user already exists in Raesum', async () => {
            ...
            vi.spyOn(raesumUser, 'setUserMetadataValues').mockResolvedValue(
                true
            );
            ...
            expect(raesumCognito.getCognitoUser).toHaveBeenCalledWith(
                mockExternalId
            );
        });
        ...
        test('should create new user if not found in Raesum', async () => {
            ...
            vi.spyOn(raesumUser, 'getUserByExternalID').mockRejectedValue(
                new Error('User not found')
            );
        ...
        test('should use external_id as username fallback if username not found in Cognito', async () => {
            vi.spyOn(raesumCognito, 'getCognitoUser').mockResolvedValue({
                UserAttributes: [{ Name: 'sub', Value: mockExternalId }],
            });
        ...
        test('should handle metadata sync failure gracefully', async () => {
            ...
            vi.spyOn(raesumUser, 'setUserMetadataValues').mockRejectedValue(
        ...
        test('should throw error if user creation fails', async () => {
            ...
            vi.spyOn(raesumUser, 'createUser').mockRejectedValue(
        ...
        test('should skip metadata sync if no matching active keys', async () => {
            ...
            const setMetadataMock = vi.spyOn(
                raesumUser,
                'setUserMetadataValues'
            );
    ...
    describe('getMetadataKeyList', () => {
        ...
        test('should handle database errors gracefully', async () => {
            vi.spyOn(raesumUser, 'getMetadataKeys').mockRejectedValue(
                new Error('Database error')
            );

            await expect(raesumUser.getMetadataKeyList()).rejects.toThrow(
                'Database error'
            );
            expect(raesumUser.getMetadataKeys).toHaveBeenCalledWith(false);
        });
        ...
        describe('cache scenarios', () => {
            test('should return cached keys when cache exists', async () => {
                const mockCachedKeys = {
                    cached_key1: { id: 1, datakey: 'cached_key1' },
                    cached_key2: { id: 2, datakey: 'cached_key2' },
                };

                vi.spyOn(raesumCache, 'get').mockResolvedValue(mockCachedKeys);
                const getMetadataKeysSpy = vi.spyOn(
                    raesumUser,
                    'getMetadataKeys'
                );
                ...
                expect(raesumCache.get).toHaveBeenCalledWith(
            ...
            test('should fetch from database when cache is empty', async () => {
                ...
                vi.spyOn(raesumUser, 'getMetadataKeys').mockResolvedValue(
                    mockKeys
                );
            ...
            test('should handle cache errors and fall back to database', async () => {
                ...
                vi.spyOn(raesumCache, 'get').mockRejectedValue(
