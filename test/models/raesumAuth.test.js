...
describe("Raesum Authorization System: Actions, Objects, and Scopes", () => {
    ...
    describe('getRolesForOrg', async () => {
        iftest('should return roles for organization with some restrictions', async () => {
    ...
    describe('getRolesByID', async () => {
        iftest('should return roles for valid IDs', async () => {
            const roles = await raesumAuthorization.getRolesByID([1, 2, 3], true);
        ...
        iftest('should handle invalid IDs mixed with valid IDs', async () => {
            const roles = await raesumAuthorization.getRolesByID([1, 99999, 2], true);
            expect(Array.isArray(roles)).toBe(true);
        ...
        iftest('should return empty array for all invalid IDs', async () => {
            const roles = await raesumAuthorization.getRolesByID([99999, 99998], true);
            expect(Array.isArray(roles)).toBe(true);
        ...
        iftest('should include inactive roles when activeOnly is false', async () => {
            const roles = await raesumAuthorization.getRolesByID([1, 999], false);
            expect(Array.isArray(roles)).toBe(true);
            // Should include both active and inactive roles
        });

        iftest('should exclude inactive roles when activeOnly is true', async () => {
            const roles = await raesumAuthorization.getRolesByID([1, 999], true);
            expect(Array.isArray(roles)).toBe(true);
    ...
    describe('checkUserPermissionByID', async () => {
        iftest('should allow access with single scope - positive case', async () => {
            // Assuming user 1 has read permission on raesum_user with user scope
            const hasPermission = await raesumAuthorization.checkUserPermissionByID(1, 1, 1, 1, 1, [1]);
            expect(hasPermission).toBe(true);
        });

        iftest('should deny access with insufficient scope - negative case', async () => {
            // Assuming user 1 does not have admin scope on raesum_user
            const hasPermission = await raesumAuthorization.checkUserPermissionByID(3, 1, 1, 1, 1, ['crossorganization']);
            expect(hasPermission).toBe(false);
        });

        iftest('should allow access with multiple scopes - positive case', async () => {
            // Test with multiple scopes where user has at least one
            const hasPermission = await raesumAuthorization.checkUserPermissionByID(1, 1, 1, 1, 1, [1, 2]);
            expect(hasPermission).toBe(true);
        });

        iftest('should deny access with multiple scopes - negative case', async () => {
            // Test with multiple scopes where user has none
            const hasPermission = await raesumAuthorization.checkUserPermissionByID(1, 1, 1, 1, 1, [412, 83]);
            expect(hasPermission).toBe(false);
        });

        iftest('should handle user with multiple roles - positive case', async () => {
            // Assuming user 2 has multiple roles with different permissions
            const hasPermission = await raesumAuthorization.checkUserPermissionByID(1, 1, 1, 1, 1);
            expect(hasPermission).toBe(true);
        });

        iftest('should handle user with multiple roles - negative case', async () => {
            // Test user with multiple roles but still lacks specific permission
            const hasPermission = await raesumAuthorization.checkUserPermissionByID(2, 1, 1, 1, 1, [2]);
            expect(hasPermission).toBe(false);
        });
        ...
        iftest('should handle deny action specifically', async () => {
            ...
            const hasPermission = await raesumAuthorization.checkUserPermissionByID(1, 1, denyActionId, 1, 1, [2]);
        });

        iftest('should handle non-existent user', async () => {
            const hasPermission = await raesumAuthorization.checkUserPermissionByID(99999, 1, 1, 1, 1, [1]);
            expect(hasPermission).toBe(false);
        ...
        iftest('should handle invalid parameters', async () => {
            expect(async () => {
                await raesumAuthorization.checkUserPermissionByID(-1, 1, 1, 1, 1);
            }).rejects.toThrow();
        });
    });
    ...
    describe('getAllowedUserScopesByID', async () => {
        iftest('should return the most powerful scope permitted for this user - positive case', async () => {
            const scopes = await raesumAuthorization.getAllowedUserScopesByID(1, 1, 1);
            expect(scopes).toBeGreaterThan(0);
        });

        iftest('should return false for user with no scopes - negative case', async () => {
            // Assuming user 999 has no permissions
            const scopes = await raesumAuthorization.getAllowedUserScopesByID(999, 1, 1);
        ...
        iftest('should handle invalid userID', async () => {
            expect(async () => {
                await raesumAuthorization.getAllowedUserScopesByID("cowgods", 1, 1, 1);
            }).rejects.toThrow();
        });
    });
    ...
    describe('addUserToRole', async () => {
        iftest('should add role to user - positive test case', async () => {
            ...
            const query = "DELETE FROM raesum.public.raesum_auth_user_x_organization_x_role WHERE user_id = 1 AND role_id = 1 AND org_id = 1;";
            const queryResult = await raesumDB.query(query);

            // Add user 1 to role 1 in org 1
            const result = await raesumAuthorization.addUserToRole(1, 1, 1);
        ...
        iftest('should fail when role not available to org', async () => {
            ...
            const mockQuery = vi.spyOn(raesumDB, 'query').mockResolvedValue({ rowCount: 0, rows: [] });
            
            // Try to add user to role that's not available for the organization
            const result = await raesumAuthorization.addUserToRole(1, 999, 1);
    ...
    describe('addUserToRoleByKey', async () => {
        iftest('should add role using valid key - positive test case', async () => {
            const query = "DELETE FROM raesum.public.raesum_auth_user_x_organization_x_role WHERE user_id = 1 AND role_id = 1 AND org_id = 1;";
            const queryResult = await raesumDB.query(query);
            
            const result = await raesumAuthorization.addUserToRoleByKey(1, 'raesumAdmin', 1);
            expect(result).toBe(true);
        });

        iftest('should fail with bad role key', async () => {
            expect(async () => {
                await raesumAuthorization.addUserToRoleByKey(1, 'nonexistent_role', 1);
            }).rejects.toThrow();
        });
    ...
    describe('getRoleByID', async () => {
        ...
        iftest('should return inactive role', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(
                expect.stringMatching(/SELECT\s+\*\s+FROM\s+raesum_auth_role\s+as\s+r\s+WHERE\s+id\s+=\s+\$1\s+AND\s+r\.active_status\s+=\s+true\s*;/),
                [999]
            );
        ...
        iftest('should handle role ID that does not exist', async () => {
            ...
            expect(async () => {
                ...
                expect(mockQuery).toHaveBeenCalledWith(
                expect.stringContaining('SELECT * FROM raesum_auth_role as r WHERE id = $1'),
                [99999]
            );
    ...
    describe('Edge Cases and Error Handling', async () => {
        iftest('should handle null parameters gracefully', async () => {
            expect(async () => {
                await raesumAuthorization.checkUserPermissionByID(null, 1, 1, 1, 1, ['user']);
            }).rejects.toThrow();
        });

        iftest('should handle undefined parameters gracefully', async () => {
            expect(async () => {
                await raesumAuthorization.checkUserPermissionByID(undefined, 1, 1, 1, 1, ['user']);
            }).rejects.toThrow();
        });
    });
