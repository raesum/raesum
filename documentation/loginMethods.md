...
3. Redirect the user to `/api/v1/auth/login?redirect_uri=URL_ENCODED_REDIRECT_URI`
    - The redirect_uri is the URI that the user will be redirected to after they have logged in from the AWS UI. It is recommended to pass the web application's URI here.
4. After the user logs in, they will be redirected back to the web application with the query string parameter `code`
5. Submit a GET request to `/api/v1/auth/callbackJWT` with the code as a query string parameter. The JWT tokens (id_token, access_token, refresh_token) will be returned in the response.
6. Store the tokens securely in your application. The access_token should be used for API calls, and the refresh_token should be stored for obtaining new tokens when the access token expires.
7. When the access token expires, submit a POST request to `/api/v1/auth/refreshJWT` with the refresh_token in the request body to obtain new tokens.
...
2. Ensure that sessions have been _SECURELY_ configured in the Raesum configuration.
